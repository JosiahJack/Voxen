...
void main() { TexCoord = aTexCoord; mat4 matrix = modelMatrices[instanceIndex]; vec4 worldPos = matrix * vec4(aPos,1.0); gl_Position = viewProjection * worldPos; FragPos = vec3(worldPos); Normal = mat3(transpose(inverse(matrix))) * aNormal; }
