...
out vec2 vUV; out vec4 vColor; out float vViewDist; flat out uint vFlags; flat out uint vTexIndex;
...
void main() {
    ...
    vec3 worldPos = pSize.xyz + uCamRight * (rotated.x * size) + uCamUp * (rotated.y * size); gl_Position = uViewProj * vec4(worldPos, 1.0); vUV = inUV; vColor = unpackColorRGBA8(d.x); vFlags = ((d.y >> 24) & 0xFFu); vTexIndex = ((d.y >> 8) & 0xFFFFu); vViewDist = dot(worldPos - uCamPos, uCamForward);
}
