...
void main() {
    int idx = gl_InstanceID + uInstanceOffset; vec4 pSize = instances[idx].posSize; float size = pSize.w; if (size <= 0.0) { gl_Position = vec4(0.0, 0.0, 2.0, 1.0); return; } /*offscreen, no div/0*/
