...
const float VOXEL_SIZE = 0.32; const uint SHADON = 2u;
uint GetVoxelIndex(vec3 worldPos) { float offsetX = worldPos.x - worldMin.x; float offsetZ = worldPos.z - worldMin.y; uint voxelX = uint(offsetX / VOXEL_SIZE); uint voxelZ = uint(offsetZ / VOXEL_SIZE); return (voxelZ * 512) + voxelX; }
const int PCF_SAMPLES = 8; const float invSamples = 1.0 / float(PCF_SAMPLES); const vec2 poissonDisk[PCF_SAMPLES] = vec2[](vec2(0.0,0.0), vec2(0.0812,0.0941),vec2(-0.0451,0.1192),vec2(-0.1221,0.0321),vec2(-0.0914,-0.0882),vec2(0.0021,-0.1275),vec2(0.1023,-0.0621), vec2(0.0452,0.0325));
vec4 getTextureColor(uint texIdx, ivec2 texCoord, int texSizeX) { uint pixelOffset = textureOffsets[texIdx] + uint(texCoord.y) * texSizeX + uint(texCoord.x); uint slotIndex = pixelOffset >> 2u; uint packedIdx = colors[slotIndex]; return unpackUnorm4x8(texturePalettes[texturePaletteOffsets[texIdx] + ((packedIdx >> ((pixelOffset & 3u) << 3u)) & 0xFFu)]); }
vec2 EncodeOctahedral(vec3 n) { n = normalize(n); vec2 p = n.xy / (abs(n.x) + abs(n.y) + abs(n.z)); return n.z >= 0.0 ? p : (1.0 - abs(p.yx)) * vec2(n.x >= 0.0 ? 1.0 : -1.0, n.y >= 0.0 ? 1.0 : -1.0); }
void GetCubemapSampleCoord(vec3 toLight, uint shadowIndex, out uint faceOff, out vec2 tc) {
    vec3 a = abs(toLight); float mx = step(a.y,a.x) * step(a.z,a.x); float my = step(a.x,a.y) * step(a.z,a.y); float oneInv_mxy = 1.0 - mx - my; vec3 mxyz = vec3(mx,my,oneInv_mxy); vec3 sxyz = vec3(step(0.0,-toLight.x),step(0.0,-toLight.y),step(0.0,-toLight.z)); vec3 fxyz = vec3(mix(1.0,0.0,sxyz.x), mix(3.0,2.0,sxyz.y), mix(5.0,4.0,sxyz.z));
    uint face = uint(mxyz.x * fxyz.x + mxyz.y * fxyz.y + mxyz.z * fxyz.z); float invMax = 1.0 / max(max(a.x,a.y),a.z); vec3 dir = -toLight * invMax; vec2 uvx = mix(vec2( dir.z, dir.y), vec2(-dir.z, dir.y), sxyz.x); vec2 uvy = mix(vec2( dir.x, dir.z), vec2( dir.x,-dir.z), sxyz.y); vec2 uvz = mix(vec2(-dir.x, dir.y), vec2( dir.x, dir.y), sxyz.z);
    vec2 uv = mx * uvx + my * uvy + oneInv_mxy * uvz; uv = uv * 0.5 + 0.5; faceOff = (shadowIndex * shadSizeSqd * 6u) + (face * shadSizeSqd); tc = uv * shadowMapSizeF;
}
...
void main() {
    ...
    if (hasNormalMap && blend > 0.01) {
        ...
        if (uvArea > 0.000000001) {
            vec3 t=dp1*duv2.y-dp2*duv1.y; t*=inversesqrt(dot(t,t)); vec3 b=dp1*duv2.x-dp2*duv1.x; b*=inversesqrt(dot(b,b)); ivec2 texSizeNorm=textureSizes[normInstanceIndex]; vec3 normalColor=(getTextureColor(normInstanceIndex,ivec2(fract(uv)*vec2(texSizeNorm)),texSizeNorm.x).rgb * 2.0 - 1.0);
            normalColor.g = -normalColor.g; vec3 transformed=mat3(t,b,adjustedNormal)*normalColor; transformed *= inversesqrt(dot(transformed,transformed)); if(dot(transformed,Normal) < 0.0)transformed=Normal; adjustedNormal=mix(Normal,transformed,blend);
        }
    }
    vec4 glowColor=vec4(0.0); if(glowIndex != 0){ivec2 texSizeGlow=textureSizes[glowIndex]; glowColor=getTextureColor(glowIndex,ivec2(fract(uv)*vec2(texSizeGlow)),texSizeGlow.x);} vec4 specColor=vec4(0.0); if(specIndex != 0){ivec2 texSizeSpec=textureSizes[specIndex]; specColor=getTextureColor(specIndex,ivec2(fract(uv)*vec2(texSizeSpec)),texSizeSpec.x);}
    if(reflectionsEnabled > 0){outSpecular=specColor; outNormal=EncodeOctahedral(adjustedNormal)*0.5 + 0.5;}/*Map to [0,1]*/ uint voxelIdx=GetVoxelIndex(worldPos); uint count=voxelLightListCounts[voxelIdx]; if(unlit>0 || useCamView>0)count=0; vec3 lighting=vec3(0.0);
    float iTot=0.0; float strength=(useStrengthMod>0) ? 1.0 : max(specColor.r,max(specColor.g,specColor.b))*4.51; float shadSizeMaxUV=shadowMapSizeF-1.0;
    for (uint i = 0u; i < count; i++) {
        uint lightIdx = uniqueLightLists[(voxelIdx * maxLightsPerVoxel) + i]; vec3 lightPos = lights[lightIdx].pos; float intensity = lights[lightIdx].intensity; float range = lights[lightIdx].range; float invRange = 1.0 / range; vec3 toLight = lightPos - worldPos; float dist = length(toLight); vec3 lightDir = toLight * (1.0 / dist);
        float NdotL = dot(adjustedNormal, lightDir); float lambertian = clamp(NdotL,0.0,1.0); float distOverRange=dist*invRange; float attenuation = (1.0 - (distOverRange * distOverRange)) * lambertian; if (attenuation < 0.03) continue; float spotAng = lights[lightIdx].spotAng; float spotFalloff = 1.0;
        if (spotAng > 0.0) { // Extremely rare, only ~15 spot lights in entire game out of several thousand lights.
            vec4 quat=lights[lightIdx].spotDir; float spotdot=dot((vec3(2.0*(quat.x*quat.z + quat.w*quat.y),2.0*(quat.y*quat.z - quat.w*quat.x),1.0 - 2.0*(quat.x*quat.x + quat.y*quat.y)))/*spotdir*/,-lightDir); float cosAngle=cos(radians(spotAng/2.0)); if(spotdot<cosAngle){continue;} spotFalloff=smoothstep(cosAngle,cos(radians(spotAng*0.8/2.0))/*Inner angle*/,spotdot); if (spotFalloff <= 0.0) continue;
        }
        float shadowFactor = 1.0; uint shadowIndex = shadowMapsIndirection[lightIdx]; bool lightHasShadows = (lights[lightIdx].lflags & SHADON) != 0u;
        if (shadowsEnabled > 0 && (shadowIndex < lightCount) && lightHasShadows) {
            uint faceOff; vec2 tc; GetCubemapSampleCoord(toLight,shadowIndex,faceOff,tc); vec2 basec = floor(tc - 0.5); vec2 coordc = clamp(basec,0.0,shadSizeMaxUV); uint ssbo_idx_center = faceOff + uint(coordc.y) * shadowMapSize + uint(coordc.x); float occDist = float(shadowMaps[ssbo_idx_center]) * 0.00001;
            float distToOccluderFac = clamp(occDist * 0.130208333, 0.0, 1.0); float smearness = distToOccluderFac * (range + intensity + 4.51) * 2.5, bias=quintic_polynomial_smoothstep(distToOccluderFac)*0.48+quintic_polynomial_smoothstep(clamp(0.16*attenuation,0.0,1.0)); float thresh=dist-bias,sum=0.0; uint threshInt=uint(max(0.0,thresh)*100000.0+0.5); 
            for (int si=0;si<PCF_SAMPLES;++si) { // Pseudo-Stochastic PCF sampling
                vec2 t = tc + poissonDisk[si] * smearness; vec2 halfT = (t - 0.5); vec2 st = fract(halfT); vec2 base = floor(halfT); uint x0 = uint(clamp(base.x, 0.0, shadSizeMaxUV)); uint y0 = uint(clamp(base.y, 0.0, shadSizeMaxUV)); uint x1 = uint(clamp(base.x + 1.0, 0.0, shadSizeMaxUV)); uint y1 = uint(clamp(base.y + 1.0, 0.0, shadSizeMaxUV));
                uint row0 = faceOff + y0 * shadowMapSize; uint row1 = faceOff + y1 * shadowMapSize; float s00 = (shadowMaps[row0 + x0] >= threshInt) ? 1.0 : 0.0; float s10 = (shadowMaps[row0 + x1] >= threshInt) ? 1.0 : 0.0; float s01 = (shadowMaps[row1 + x0] >= threshInt) ? 1.0 : 0.0; float s11 = (shadowMaps[row1 + x1] >= threshInt) ? 1.0 : 0.0;
