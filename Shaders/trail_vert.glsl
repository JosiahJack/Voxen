...
layout(std430, binding = 11) readonly buffer TrailInstanceBlock {
    vec4 p0Width[4096];
    vec4 p1Width[4096];
    uvec4 data[4096];
};
...
void main() {
    ...
    float along = inCorner.x;
    float side = inCorner.y;

    vec3 center = mix(p0.xyz, p1.xyz, along);
    float width = mix(p0.w, p1.w, along);

    vec3 segmentDir = normalize(p1.xyz - p0.xyz);
    vec3 viewDir = normalize(uCamPos - center);

    vec3 sideVec = cross(segmentDir, viewDir);
    float lenSq = dot(sideVec, sideVec);
    if (lenSq < 1e-8) {
        sideVec = uCamRight;
    } else {
    ...
    vec3 worldPos = center + sideVec * (side * width);

    gl_Position = uViewProj * vec4(worldPos, 1.0);

    vUV = vec2(along, side * 0.5 + 0.5);
    uvec4 d = data[i];
    vColor = mix(unpackColorRGBA8(d.x), unpackColorRGBA8(d.y), along);
}
