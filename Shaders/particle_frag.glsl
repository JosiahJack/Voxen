// particle_frag.glsl - Particle fragment shader using the engine's SSBO palette texture system (same as chunk_frag)
...
vec4 getParticleTextureColor(ivec2 texCoord, int texSizeX) {
    uint pixelOffset = textureOffsets[uTexIndex] + uint(texCoord.y) * uint(texSizeX) + uint(texCoord.x);
    ...
    uint paletteOffset = texturePaletteOffsets[uTexIndex];
    uint color = texturePalettes[paletteOffset + paletteIndex];
    return unpackUnorm4x8(color);
}

float linearizeSceneDepth(vec2 screenUV) {
    float rawDepth = texture(uSceneDepth, screenUV).r;
    float ndcZ = rawDepth * 2.0 - 1.0;
    float near = 0.1;
    float far = 500.0;
    float viewZ = (2.0 * near * far) / (far + near - ndcZ * (far - near));
    return viewZ;
}

void main() {
    ivec2 texSize = textureSizes[uTexIndex];
    vec2 uv = vec2(vUV.x, 1.0 - vUV.y);
    vec4 texColor = getParticleTextureColor(ivec2(clamp(fract(uv), 0.0, 0.99999) * vec2(texSize)), texSize.x);

    if (uBlendMode == 1) { // Additive (brightness-as-alpha, adds to scene)
        outColor = vec4(texColor.rgb * texColor.a, 0.0);
        return;
    }

    if (uBlendMode == 2) { // Multiply (darkens where texture is dark)
