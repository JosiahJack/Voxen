...
void InputCursorPos(double* x, double* y, double xpos, double ypos) {
    ...
    if ((World.inventoryMode && !Cheats.noHUD) || World.menuActive || World.paused) {/*Uses UI baseline resolution 1366x768*/ i32 newX = clamp(World.cursorPos_x + World.currentMouse_dx,0,UI_W); if (newX != World.cursorPos_x) {mouseMovementThisFrame = true;} World.cursorPos_x = newX; i32 newY = clamp(World.cursorPos_y + World.currentMouse_dy,0,UI_H); if (newY != World.cursorPos_y) {mouseMovementThisFrame = true;} World.cursorPos_y = newY;}
}
...
void InputProcessing() {
    ...
    if (window_has_focus) {
        ...
        if (Biomonitor()) { if (!(World.invP1.hasHardware & HW_BIO) || (World.invP1.energy<=0.0f && World.invP1.hwVersSetting[HW_BIO_IDX]==0)) CenterStatusPrint("%s",Sys_Text.stringTable[314]); else { play_wav(sounds[(World.invP1.hardwareIsActive&HW_BIO)?78:78],SfxVol(),(V3){0,0,0},false); World.invP1.hardwareIsActive ^= HW_BIO; if ((World.invP1.hardwareIsActive&HW_BIO)==0) BioMonitorClearGraphs(); } }
        if (Sensaround()) { if (!(World.invP1.hasHardware & HW_SNS) || World.invP1.energy<=0.0f) CenterStatusPrint("%s",Sys_Text.stringTable[314]); else { play_wav(sounds[(World.invP1.hardwareIsActive&HW_SNS)?82:93],SfxVol(),(V3){0,0,0},false); World.invP1.hardwareIsActive ^= HW_SNS; } }
        if (Shield()) { if (!(World.invP1.hasHardware & HW_SHD) || World.invP1.energy<=0.0f) CenterStatusPrint("%s",Sys_Text.stringTable[314]); else { play_wav(sounds[(World.invP1.hardwareIsActive&HW_SHD)?95:96],SfxVol(),(V3){0,0,0},false); World.invP1.hardwareIsActive ^= HW_SHD; } }
        if (Email()) { if (World.invP1.hasHardware & HW_ERD) { MFD_ResetGeneral(); World.Sys_UI.MFD_CenterTab=5; World.Sys_UI.MFD_LefTab=2; World.Sys_UI.mfdItemReader[0]=true; World.Sys_UI.MFD_ReaderView=MFD_READER_CONTENTS; World.Sys_UI.MFD_MediaTab=World.Sys_UI.lastMultiMediaTabOpened; if (World.Sys_UI.MFD_MediaTab>MM_NOTES || (World.Sys_UI.MFD_MediaTab==MM_NOTES && !World.diffMis)) World.Sys_UI.MFD_MediaTab=MM_LOG_TABLE; play_wav(sounds[97],SfxVol(),(V3){0,0,0},false); } }
        if (Booster()) { if (!(World.invP1.hasHardware & HW_BST) || (World.invP1.energy<=0.0f && World.invP1.hwVersSetting[HW_BST_IDX]>=1)) CenterStatusPrint("%s",Sys_Text.stringTable[314]); else { play_wav(sounds[(World.invP1.hardwareIsActive&HW_BST)?78:78],SfxVol(),(V3){0,0,0},false); World.invP1.hardwareIsActive ^= HW_BST; } }
        if (Jumpjets()) { if (!(World.invP1.hasHardware & HW_JET) || World.invP1.energy<=0.0f) CenterStatusPrint("%s",Sys_Text.stringTable[314]); else { play_wav(sounds[(World.invP1.hardwareIsActive&HW_JET)?78:78],SfxVol(),(V3){0,0,0},false); World.invP1.hardwareIsActive ^= HW_JET; } }
