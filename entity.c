...
void LoadLevelMod(u8 lev) {
    ...
    for (i32 e=0;e<totalEnts;++e) {
        ...
        par->timeInterval          = src->timeInterval;
        par->randomMin             = src->randomMin;
        par->randomMax             = src->randomMax;
        par->useRandomTimes        = src->useRandomTimes;
        par->intervalFinished      = src->intervalFinished;
        par->health                = src->health;
        par->cyberHealth           = src->cyberHealth;
        par->radiation             = src->radiation;
        par->gracePeriodFinished   = src->gracePeriodFinished;
        par->meleeDamageFinished   = src->meleeDamageFinished;
        par->idleTime              = src->idleTime;
        par->attack1SoundTime      = src->attack1SoundTime;
        par->attack2SoundTime      = src->attack2SoundTime;
        par->attack3SoundTime      = src->attack3SoundTime;
        par->deathBurstFinished    = src->deathBurstFinished;
        par->tranquilizeFinished   = src->tranquilizeFinished;
        par->wanderFinished        = src->wanderFinished;
        par->timeSinceMovedEnough  = src->timeSinceMovedEnough;
        par->posCheckFinished      = src->posCheckFinished;
        par->currentFrameFinished  = src->currentFrameFinished;
        par->animFinished          = src->animFinished;
        par->animSwapFinished      = src->animSwapFinished;
        par->delayFireFinished     = src->delayFireFinished;
        par->delayResetFinished    = src->delayResetFinished;
        par->delayFinished         = src->delayFinished;
