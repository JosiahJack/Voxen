...
static GLFWbool waitForAnyEvent(double* timeout) {
    ...
    struct pollfd fds[] = {[XLIB_FD] = {ConnectionNumber(_glfw.x11.display),POLLIN,0U},[PIPE_FD]={_glfw.x11.emptyEventPipe[0],POLLIN,0U},[INOTIFY_FD]={-1,POLLIN,0U}};
    ...
    while (!XPending(_glfw.x11.display)) {
        ...
        for (int i=1;i<(int)(sizeof(fds)/sizeof(fds[0]));++i) {
...
static Bool isSelectionEvent(Display* display, XEvent* event, XPointer pointer) {
    (void)display; (void)pointer; if (event->xany.window != _glfw.x11.helperWindowHandle) return False;
...
static Bool isSelPropNewValueNotify(Display* display, XEvent* event, XPointer pointer) {
    XEvent* notification = (XEvent*) pointer; (void)display;
    return event->type == PropertyNotify && event->xproperty.state == PropertyNewValue && event->xproperty.window == notification->xselection.requestor && event->xproperty.atom == notification->xselection.property;
}
...
void _glfwPushSelectionToManagerX11(void) {
    XConvertSelection(_glfw.x11.display,_glfw.x11.CLIPBOARD_MANAGER,_glfw.x11.SAVE_TARGETS,None,_glfw.x11.helperWindowHandle,CurrentTime);
    for (;;) {
        XEvent event;
        while (XCheckIfEvent(_glfw.x11.display, &event, isSelectionEvent, NULL)) {
            switch (event.type) {
                case SelectionRequest: handleSelectionRequest(&event); break;
                case SelectionNotify: if (event.xselection.target == _glfw.x11.SAVE_TARGETS) {return;} break;
            }
        }
...
void _glfwSetWindowAspectRatioX11(_GLFWwindow* window, int numer, int denom) {
    int width, height; (void)denom; (void)numer;
    _glfwGetWindowSizeX11(window,&width,&height);
    updateNormalHints(window,width,height);
    XFlush(_glfw.x11.display);
}
...
void _glfwSetWindowFloatingX11(_GLFWwindow* window, GLFWbool enabled) {
    ...
    if (_glfwWindowVisibleX11(window)) {
        ...
        if (i < count) {
            ...
            XChangeProperty(_glfw.x11.display,window->x11.handle,_glfw.x11.NET_WM_STATE,XA_ATOM,32,PropModeReplace,(unsigned char*)states,count - 1);
        }
...
GLFWAPI Display* glfwGetX11Display(void) {
    _GLFW_REQUIRE_INIT_OR_RETURN(NULL); if (_glfw.platform.platformID != GLFW_PLATFORM_X11) { _glfwInputError(GLFW_PLATFORM_UNAVAILABLE, "X11: Platform not initialized"); return NULL; }
