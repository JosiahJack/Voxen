...
int pcm_params_refine(int fd, pcm_params_t *params);
...
static inline int pcm_prepare(int fd) { return OS_IOControlSimple(fd,PCM_ACTION_PREPARE); }
static inline int pcm_start(int fd)   { return OS_IOControlSimple(fd,PCM_ACTION_START); }
static inline int pcm_stop(int fd)    { return OS_IOControlSimple(fd,PCM_ACTION_STOP); }
static inline int pcm_drain(int fd)   { return OS_IOControlSimple(fd,PCM_ACTION_DRAIN); }
static inline int pcm_xrun(int fd)    { return OS_IOControlSimple(fd,PCM_ACTION_XRUN); }
static inline int pcm_reset(int fd)   { return OS_IOControlSimple(fd,PCM_ACTION_RESET); }
static inline int pcm_resume(int fd)  { return OS_IOControlSimple(fd,PCM_ACTION_RESUME); }
static inline int pcm_pause(int fd)   { return OS_IOControl(fd,PCM_ACTION_PAUSE,(void*)1); }
static inline int pcm_unpause(int fd) { return OS_IOControl(fd,PCM_ACTION_PAUSE,(void*)0); }
static inline int pcm_mmap_sync_pos(int fd) { return OS_IOControlSimple(fd, PCM_MMAP_SYNC_POSITION); }
static inline int pcm_move_app_pos(int fd, int frames) { return frames < 0 ? OS_IOControl(fd,PCM_DO_REWIND,(void*)-frames) : OS_IOControl(fd,PCM_DO_FORWARD,(void*)frames); }
static inline int pcm_link(int fd, int fd2) { return OS_IOControl(fd,PCM_DO_LINK,(void*)fd2); }
static inline int pcm_unlink(int fd) { return OS_IOControlSimple(fd,PCM_DO_UNLINK); }
...
static void hw_params_set_mask(struct snd_pcm_hw_params *p, int parameter, unsigned int value) { struct snd_mask *m = get_mask_struct(p,parameter); if (m->bits[get_index(value)] & get_mask(value)) {SetMemoryToValueForNBytes(m, 0x00, sizeof(*m));} m->bits[get_index(value)] |= get_mask(value); }
...
static void hw_params_fill(struct snd_pcm_hw_params *p) {
    int i;
    SetMemoryToValueForNBytes(p,0,sizeof(*p));
    SetMemoryToValueForNBytes(p->masks,0xff,sizeof(p->masks));
...
void pcm_params_init(pcm_params_t *p) {
	...
	SetMemoryToValueForNBytes(sw, 0, sizeof(*sw));
...
void pcm_set(pcm_params_t *params, pcm_param_t parameter, unsigned long value) {
	...
	case PCM_INTERRUPT: hw->flags = value ? hw->flags|PCM_NO_INTERRUPTS : hw->flags & ~PCM_NO_INTERRUPTS; break;
	case PCM_TSTAMP_TYPE: sw->tstamp_mode = SNDRV_PCM_TSTAMP_ENABLE; sw->tstamp_type = value; break;
...
int pcm_open(int card, int device, int flags) {
	char path[4096];
	StringFormat(path,sizeof(path),PCM_DEV_PATH "pcmC%uD%u%c",card,device,(flags&1)==PCM_INPUT ? 'c' : 'p');
	return OS_Open(path,O_RDWR|(flags&PCM_NONBLOCK?O_NONBLOCK:0),0);
}
