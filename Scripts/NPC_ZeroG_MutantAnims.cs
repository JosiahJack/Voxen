    ...
    void Update() {
		if (!Eng_Global->gamePaused && !Eng_Global->menuActive) {
			...
			if (tickFinished < Eng_Global->pauseRelativeTime) {
				...
				if ((tickFinished - Eng_Global->pauseRelativeTime) > (tickTime * 2f)) {
					currentFrame += (int) (vceil((tickFinished - Eng_Global->pauseRelativeTime)/tickTime)); // increment by however many frames we skipped, e.g. slower than 24fps
				} else {
				...
				tickFinished = Eng_Global->pauseRelativeTime + tickTime;
			}
		}
    }
}
