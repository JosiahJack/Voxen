public class LogDataTabContainerManager : MonoBehaviour {
	...
	void SendLogData(int referenceIndex, bool isRH) {
		...
		if (!isRH) {
			logSender.text = Eng_Text->stringTable[893] + Const.a.audiologSenders[referenceIndex];
			logSubject.text = Eng_Text->stringTable[894] + Environment.NewLine + Const.a.audiologSubjects[referenceIndex];
