public class QuestLogNotesManager : MonoBehaviour {
	...
	void Awake() {
		...
		if (Inventory.a != null) Eng_Global->inventoryPlayer1.hasNewNotes = true;
	}
	...
	void NotifyLockedDoorAttempt(Door d) {
		if (d == level6elevatorDoorTo7) {
			...
			labels[12].text = Eng_Text->stringTable[565];
			if (Inventory.a != null) Eng_Global->inventoryPlayer1.hasNewNotes = true;
		}
	}
	...
	public void LogAdded(int logCustomIndex) {
		...
		switch (logCustomIndex) {
			case 6: // #2-4601-06.MAY.72, just rewards
				...
				Eng_Global->inventoryPlayer1.hasNewNotes = true;
				labels[6].text += Eng_Text->stringTable[555]; // Add:  Keycode is 451.
				break;
			case 8: // Honig-11.OCT.72, Medical CPU's
				notes[0].SetActive(true);
				Eng_Global->inventoryPlayer1.hasNewNotes = true;
				labels[0].text = Eng_Text->stringTable[556] + "1" + Eng_Text->stringTable[557]; // Set:Destroy level 1 nodes.
				break;
			...
			case 15: // D'Arcy-23.OCT.72, destroying the laser
				...
				Eng_Global->inventoryPlayer1.hasNewNotes = true;
				labels[7].text = Eng_Text->stringTable[559]; // Set:Disengage laser safety override.
				labels[8].text = Eng_Text->stringTable[560]; // Set:Activate the station energy shield.
				labels[9].text = Eng_Text->stringTable[561]; // Set:Destroy the mining laser.
				break;
			...
			case 42: // diagnostic-06.NOV.72, repair diagnostic
				...
				Eng_Global->inventoryPlayer1.hasNewNotes = true;
				labels[10].text = Eng_Text->stringTable[562]; // Set:Enable master jettison.
				labels[11].text = Eng_Text->stringTable[563]; // Set:Diagnose and repair broken relay
				labels[11].text += Eng_Text->stringTable[564]; // Add:: 428
				break;
			case 66: // SHODAN-07.OCT.72, new jettison procedure
				...
				Eng_Global->inventoryPlayer1.hasNewNotes = true;
				labels[10].text = Eng_Text->stringTable[562]; // Set:Enable master jettison.
				labels[12].text = Eng_Text->stringTable[565]; // Set:Diagnose and repair broken relay
				break;
			...
			case 90: // Rebecca-3, SHODAN downloading
				notes[13].SetActive(true); // Destroy the four relay antennae.
				Eng_Global->inventoryPlayer1.hasNewNotes = true;
				labels[13].text = Eng_Text->stringTable[566]; // Set:Destroy the four relay antennae.
				break;
			case 93: // Rebecca-4, SHODAN destroying Citadel Station
				notes[14].SetActive(true);
				Eng_Global->inventoryPlayer1.hasNewNotes = true;
				labels[14].text = Eng_Text->stringTable[567]; // Set:Destroy the mining laser.
				break;
			case 97: // Rebecca-5, get to the bridge!
				notes[16].SetActive(true);
				notes[17].SetActive(true);
				Eng_Global->inventoryPlayer1.hasNewNotes = true;
				labels[16].text = Eng_Text->stringTable[569]; // Set:Access the bridge.
				labels[17].text = Eng_Text->stringTable[570]; // Set:Destroy SHODAN.
				label15_StrikeThru.SetActive(true);
				break;
			case 120: // VMailStatus, Shield Activated
				notes[8].SetActive(true);
				Eng_Global->inventoryPlayer1.hasNewNotes = true;
				labels[8].text = Eng_Text->stringTable[569]; // Set:Activate the station energy shield.
	...
	public void NodesDestroyed(int levelIndex) {
		...
		switch (levelIndex) {
			case 1: // Level 1
				notes[0].SetActive(true);
				labels[0].text = Eng_Text->stringTable[556] + "1" + Eng_Text->stringTable[557]; // Set:Destroy level 1 nodes. // Make sure this was on.
				if (!notes[1].activeInHierarchy) { // Level 2
					notes[1].SetActive(true);
					labels[1].text = Eng_Text->stringTable[556] + "2" + Eng_Text->stringTable[557]; // Set:Destroy level 2 nodes.
				}
				
				checkBoxes[0].isOn = true;
				Eng_Global->inventoryPlayer1.hasNewNotes = true;
				labels[0].text += Eng_Text->stringTable[558] + Const.a.questData.lev1SecCode.ToString() + "."; // Add:  Code is , Add:#, Add:.
				break;
			case 2: // Level 2
				notes[1].SetActive(true);
				labels[1].text = Eng_Text->stringTable[556] + "2" + Eng_Text->stringTable[557]; // Set:Destroy level 2 nodes. // Make sure this was on.
				if (!notes[2].activeInHierarchy) { // Level 3
					notes[2].SetActive(true);
					labels[2].text = Eng_Text->stringTable[556] + "3" + Eng_Text->stringTable[557]; // Set:Destroy level 3 nodes.
				}
				
				checkBoxes[1].isOn = true;
				Eng_Global->inventoryPlayer1.hasNewNotes = true;
				labels[1].text += Eng_Text->stringTable[558] + Const.a.questData.lev2SecCode.ToString() + "."; // Add:  Code is , Add:#, Add:.
				break;
			case 3: // Level 3
				notes[2].SetActive(true);
				labels[2].text = Eng_Text->stringTable[556] + "3" + Eng_Text->stringTable[557]; // Set:Destroy level 3 nodes. // Make sure this was on.
				if (!notes[3].activeInHierarchy) { // Level 4
					notes[3].SetActive(true);
					labels[3].text = Eng_Text->stringTable[556] + "4" + Eng_Text->stringTable[557]; // Set:Destroy level 4 nodes.
				}
								
				checkBoxes[2].isOn = true;
				Eng_Global->inventoryPlayer1.hasNewNotes = true;
				labels[2].text += Eng_Text->stringTable[558] + Const.a.questData.lev3SecCode.ToString() + "."; // Add:  Code is , Add:#, Add:.
				break;
			case 4: // Level 4
				notes[3].SetActive(true);
				labels[3].text = Eng_Text->stringTable[556] + "4" + Eng_Text->stringTable[557]; // Set:Destroy level 4 nodes. // Make sure this was on.
				if (!notes[4].activeInHierarchy) { // Level 5
					notes[4].SetActive(true);
					labels[4].text = Eng_Text->stringTable[556] + "5" + Eng_Text->stringTable[557]; // Set:Destroy level 5 nodes.
				}
				
				checkBoxes[3].isOn = true;
				Eng_Global->inventoryPlayer1.hasNewNotes = true;
				labels[3].text += Eng_Text->stringTable[558] + Const.a.questData.lev4SecCode.ToString() + "."; // Add:  Code is , Add:#, Add:.
				break;
			case 5: // Level 5
				notes[4].SetActive(true);
				labels[4].text = Eng_Text->stringTable[556] + "5" + Eng_Text->stringTable[557]; // Set:Destroy level 5 nodes. // Make sure this was on.
				if (!notes[5].activeInHierarchy) { // Level 6
					notes[5].SetActive(true);
					labels[5].text = Eng_Text->stringTable[556] + "6" + Eng_Text->stringTable[557]; // Set:Destroy level 6 nodes.
				}
				
				checkBoxes[4].isOn = true;
				Eng_Global->inventoryPlayer1.hasNewNotes = true;
				labels[4].text += Eng_Text->stringTable[558] + Const.a.questData.lev5SecCode.ToString() + "."; // Add:  Code is , Add:#, Add:.
				break;
			case 6: // Level 6
				notes[5].SetActive(true);
				labels[5].text = Eng_Text->stringTable[556] + "6" + Eng_Text->stringTable[557]; // Set:Destroy level 6 nodes. // Make sure this was on.
				checkBoxes[5].isOn = true;
				Eng_Global->inventoryPlayer1.hasNewNotes = true;
				labels[5].text += Eng_Text->stringTable[558] + Const.a.questData.lev6SecCode.ToString() + "."; // Add:  Code is , Add:#, Add:.
				break;
		}
	}
