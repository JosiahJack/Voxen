...
void LoadLevel(uint8_t curlevel) {
    ...
    if (!Sys_Global.levelCurrentlyLoading) __builtin_memset(Sys_Global.instances + 3,0,(INSTANCE_COUNT - 3) * sizeof(Entity)); // Initialize instances, the global entity array for the currently loaded level.
    Sys_Global.levelCurrentlyLoading = true;
        ...
        while(line[0] != '\0') { // Guaranteed no leading whitespaces,k comments, or blank lines, so don't bother
            ...
            *colon = '\0';           // Split string at the colon
            ...
            if (isLight) {
                ...
                Entity* inst = &Sys_Global.instances[instanceIdx];
...
//                 else if (StringsAreEqual(trimmed_key,"doorOpenState"))   inst->doorOpen = parse_numberu8(trimmed_value, initialLine, lineNum);
                ...
                else if (StringsAreEqual(trimmed_key,"useFinished"))     inst->useFinished = parse_float(trimmed_value, initialLine, lineNum) + Sys_Global.pauseRelativeTime;
                else if (StringsAreEqual(trimmed_key,"waitBeforeClose")) inst->waitBeforeClose = parse_float(trimmed_value, initialLine, lineNum) + Sys_Global.pauseRelativeTime;
                else if (StringsAreEqual(trimmed_key,"lasersFinished"))  inst->lasersFinished = parse_float(trimmed_value, initialLine, lineNum) + Sys_Global.pauseRelativeTime;
