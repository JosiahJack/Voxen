...
#include "common.h"
u16 cellLists[WORLDX*WORLDX][128],cellCounts[WORLDX*WORLDX]; u16 triggerVolumes[128]; u16 numTriggers; extern double game_actual_start_time; static bool ladderWalkOff = false; static float ladderTopY = 0.0f; static double ladderSFXFinished = 0.0; void DrawBoxColliderColored(u16 i, Color col); void DrawCapsuleCollider(u16 i);
u16 GetTeleportDestination(u8 level,u16 teleportID);
...
static const float WALK_SPEED=5.7f,PLAYER_MAX_CYBER_SPEED=10.0f,CROUCH_SPEED=2.5f,PLAYER_MAX_PRONE_SPEED=1.6f,PLAYER_BOOSTER_SPEED_BOOST=1.2f,PLAYER_CROUCH_RATIO=0.63f,PLAYER_PRONE_RATIO=0.32f,WALK_SPEED_ACCEL=3.0f,BOOST_ACCEL=1.0f,SKATE_SLIDE=5.0f,SKATE_SPEED=1.1f;/*SKATE_SLIDE is Citadel's 0.5/0.1 walkDeaccelerationBooster ratio; accel converges velocity toward target so dividing by it is the slippery direction, and larger means more slide just as Citadel's smoothTime does*/
...
static bool IsImpactProjectile(u16 idx) { return idx >= 481 && idx <= 495; }
static void QueueProjectileImpact(u16 projectile,u16 other,V3 point,V3 normal){if(projectile>=World.instCount||other>=World.instCount||other==World.instances[projectile].recentMostActivator)return;projectileContactThisFrame[projectile]=1;if(World.instances[projectile].currentTargetIdx==(u16)(other+1))return;if(!projectileImpactQueued[projectile]){projectileImpactQueued[projectile]=1;projectileImpactTarget[projectile]=other;projectileImpactPoint[projectile]=point;projectileImpactNormal[projectile]=normal;}}
...
void CyberItemOnTriggerEnter(u16 self, u16 other) {
    if(other!=PLAYER1){return;} bool success=false;
    switch(World.instances[self].index) {
        case 450/*item_cyber_drill*/: if(World.invP1.isPulserNotDrill&&!(World.invP1.hasSoft&(1u<<SW_PULSER))){World.invP1.isPulserNotDrill=false;} if(World.instances[self].version>World.invP1.softVersions[SW_DRILL]){World.invP1.softVersions[SW_DRILL]=(u8)World.instances[self].version;}else{CenterStatusPrint("%s",Sys_Text.stringTable[46]);} World.invP1.hasSoft|=(1u<<SW_DRILL); play_wav(sounds[86], AppliedFXVol(1.0f), (V3){0.0f,0.0f,0.0f}, false); CenterStatusPrint("%s%d%s",Sys_Text.stringTable[444],World.invP1.softVersions[SW_DRILL],Sys_Text.stringTable[458]); success=true; break;
        case 454/*item_cyber_pulser*/: if(!World.invP1.isPulserNotDrill&&!(World.invP1.hasSoft&(1u<<SW_PULSER))){World.invP1.isPulserNotDrill=true;} if(World.instances[self].version>World.invP1.softVersions[SW_PULSER]){World.invP1.softVersions[SW_PULSER]=(u8)World.instances[self].version;}else{CenterStatusPrint("%s",Sys_Text.stringTable[46]);} World.invP1.hasSoft|=(1u<<SW_PULSER); play_wav(sounds[86], AppliedFXVol(1.0f), (V3){0.0f,0.0f,0.0f}, false); CenterStatusPrint("%s%d%s",Sys_Text.stringTable[445],World.invP1.softVersions[SW_PULSER],Sys_Text.stringTable[458]); success=true; break;
        case 456/*item_cyber_shield*/: if(World.instances[self].version>World.invP1.softVersions[SW_SHIELD]){World.invP1.softVersions[SW_SHIELD]=(u8)World.instances[self].version;}else{CenterStatusPrint("%s",Sys_Text.stringTable[46]);} World.invP1.hasSoft|=(1u<<SW_SHIELD); play_wav(sounds[86], AppliedFXVol(1.0f), (V3){0.0f,0.0f,0.0f}, false); CenterStatusPrint("%s%d%s",Sys_Text.stringTable[446],World.invP1.softVersions[SW_SHIELD],Sys_Text.stringTable[458]); success=true; break;
        case 457/*item_cyber_turbo*/: if(World.invP1.cyberItemIndex<0){World.invP1.cyberItemIndex=0;} World.invP1.softVersions[SW_TURBO]++; World.invP1.hasSoft|=(1u<<SW_TURBO); play_wav(sounds[86], AppliedFXVol(1.0f), (V3){0.0f,0.0f,0.0f}, false); CenterStatusPrint("%s",Sys_Text.stringTable[447]); success=true; break;
        case 449/*item_cyber_decoy*/: if(World.invP1.cyberItemIndex<0){World.invP1.cyberItemIndex=1;} World.invP1.softVersions[SW_DECOY]++; World.invP1.hasSoft|=(1u<<SW_DECOY); play_wav(sounds[86], AppliedFXVol(1.0f), (V3){0.0f,0.0f,0.0f}, false); CenterStatusPrint("%s",Sys_Text.stringTable[448]); success=true; break;
        case 455/*item_cyber_recall*/: if(World.invP1.cyberItemIndex<0){World.invP1.cyberItemIndex=2;} World.invP1.softVersions[SW_RECALL]++; World.invP1.hasSoft|=(1u<<SW_RECALL); play_wav(sounds[86], AppliedFXVol(1.0f), (V3){0.0f,0.0f,0.0f}, false); CenterStatusPrint("%s",Sys_Text.stringTable[449]); success=true; break;
        case 451/* ;) item_cyber_game*/: { int vers=World.instances[self].version; if(vers<0||vers>=7){break;} World.invP1.hasNewData=true; World.invP1.hasMinigame|=(u8)(1u<<vers); static const u16 gameMsg[7]={450,451,452,453,454,455,456}; play_wav(sounds[86], AppliedFXVol(1.0f), (V3){0.0f,0.0f,0.0f}, false); CenterStatusPrint("%s",Sys_Text.stringTable[gameMsg[vers]]); success=true; break; }
        case 448/*item_cyber_data*/: { int vers=World.instances[self].version; World.invP1.hasNewData=true; if(vers>=0&&vers<LOGCNT){World.invP1.hasLog[vers]=true;} play_wav(sounds[87], AppliedFXVol(1.0f), (V3){0.0f,0.0f,0.0f}, false); CenterStatusPrint("%s",Sys_Text.stringTable[457]); success=true; break; }
        case 452/*item_cyber_integrity*/: { Entity* player=&World.instances[PLAYER1]; if(player->cyberHealth>=255.0f){break;} play_wav(sounds[86], AppliedFXVol(1.0f), (V3){0.0f,0.0f,0.0f}, false); player->cyberHealth+=77.0f; if(player->cyberHealth>255.0f){player->cyberHealth=255.0f;} CenterStatusPrint("%s",Sys_Text.stringTable[459]); success=true; break; }
...
// TeleportTouch
void TeleportTouchOnTriggerEnter(u16 self, u16 o){Entity *e=&World.instances[self],*player=&World.instances[PLAYER1]; if(!e->touchEnabled||o!=PLAYER1||player->health<=0.0f||e->justUsed>=World.pauseRelativeTime)return; u16 dest=GetTeleportDestination(World.currentLevel,e->targetDestinationID); if(dest>=World.instCount||World.instances[dest].index!=703)return; World.position[PLAYER1]=World.position[dest]; World.instances[dest].justUsed=World.pauseRelativeTime+1.0; play_wav(sounds[106], AppliedFXVol(1.0f), World.position[dest], false);}
...
void TriggerTriggerTripped(u16 self, u16 other) { Entity* e=&World.instances[self]; if(other != PLAYER1 || (World.instances[self].allDone && World.instances[self].onlyOnce)) return; e->recentMostActivator=other; if(e->onlyOnce){e->allDone=true;} if(e->delay <= 0.0f){UseTargets(self,World.instances[self].targetIdx);}else{TriggerDelayedTarget(self);} }
...
static float GravityLiftTopY(u16 liftIdx) {
    ShapeBox box=Entity_GetBox(liftIdx); V3 up=quat_rot_v3(box.rot,(V3){0,1,0});
    float extent=vabs(up.x)*box.hExt.x+vabs(up.y)*box.hExt.y+vabs(up.z)*box.hExt.z;
    return box.ctr.y+extent;
}
static float GravityLiftBodyBottomY(u16 body) {
    if(World.col[body]==COLTYPE_CAP){ShapeCapsule cap=Entity_GetCap(body);return cap.base.y-cap.rad;}
    return World.position[body].y-(World.colliderSize[body].y*0.5f*vabs(World.scale[body].y));
}
static float GravityLiftGravityAccel(u16 body) {
    if(body==PLAYER1&&(Cheats.noclip||World.invP1.ladderState>0))return 0.0f;
    return 9.81f*World.gravity[body];
}
static float GravityLiftTopControlForce(u16 body,float gap) {
    /* Underdamped spring toward the top plane; at rest this exactly balances gravity. */
    float accel=vclamp(gap*64.0f-World.velocity[body].y*6.0f,-32.0f,20.0f);
    return vmax(World.mass[body],0.001f)*(GravityLiftGravityAccel(body)+accel);
}
static void GravityLiftApplyTopSupport(u16 body) {
    u16 marker=gravityLiftTopSupport[body]; if(!marker)return; u16 liftIdx=(u16)(marker-1u);
    if(liftIdx<INSTS_1ST_IDX||liftIdx>=World.instCount||World.instances[liftIdx].index!=596||!World.instances[liftIdx].active){gravityLiftTopSupport[body]=0;return;}
    ShapeBox box=Entity_GetBox(liftIdx); ShapeCapsule cap=Entity_GetCap(body); V3 delta=V3_AsubB(World.position[body],box.ctr),ax=quat_rot_v3(box.rot,(V3){1,0,0}),az=quat_rot_v3(box.rot,(V3){0,0,1});
    float lx=V3_dot(delta,ax),lz=V3_dot(delta,az),gap=GravityLiftTopY(liftIdx)-(cap.base.y-cap.rad);
    if(vabs(lx)>box.hExt.x+cap.rad||vabs(lz)>box.hExt.z+cap.rad||gap>1.0f||gap < -0.24f){gravityLiftTopSupport[body]=0;return;}
    bool topContact=gap<=0.08f&&gap>=-0.12f,topJumpZone=gap<=vmax(World.instances[liftIdx].distancePaddingToTopPoint,0.12f)&&gap>=-0.12f;
    if(body==PLAYER1&&topJumpZone&&JumpDown()){gravityLiftTopSupport[body]=0;flag_set(&World.instances[body].entflags,EF_GROUNDED,true);return;}
    if(body==PLAYER1&&topContact&&vabs(World.velocity[body].y)<0.75f)flag_set(&World.instances[body].entflags,EF_GROUNDED,true);
    if(gravityLiftTouched[body]!=marker)AddForce(body,(V3){0.0f,GravityLiftTopControlForce(body,gap),0.0f},false);
}
void trigger_gravitylift_touch(u16 self, u16 other, bool entering) {
    ...
    if(lift->active){
        ...
        if(gap<=vmax(lift->distancePaddingToTopPoint,stopDist+0.20f)){
            ...
            if(!jumpingFromTop)forceY=GravityLiftTopControlForce(other,gap);
        }else if(initialBurst){
            /* Preserve the tuned entry burst. */
            forceY=(lift->strength*World.mass[other]-velY)*2.0f;
        }else{
            /* Track a useful upward speed while cancelling gravity; the former force reached a slow terminal rise under gravity. */
            float riseAccel=vclamp((lift->strength-velY)*3.0f,-16.0f,12.0f);
            forceY=mass*(gravityAccel+riseAccel);
        }
...
INLINE Overlap CapCap(ShapeCapsule a, ShapeCapsule b) {
    ...
    if(qa<PHY_EPSILON && e < PHY_EPSILON){sc=tc=0.0f;} else if(qa < PHY_EPSILON){sc=0.0f; tc=vclamp(f/e,0.0f,1.0f);}else{float c=V3_dot(d1,vr); if(e<PHY_EPSILON){tc=0.0f; sc=vclamp(-c/qa,0.0f,1.0f);}else{float qb=V3_dot(d1,d2),denom=qa*e-qb*qb; sc=(denom > PHY_EPSILON) ? vclamp((qb*f - c*e)/denom,0.0f,1.0f) : 0.0f; tc=(qb*sc + f)/e; if(tc < 0.0f){tc=0.0f; sc=vclamp(-c/qa,0.0f,1.0f);}else if(tc > 1.0f){tc=1.0f; sc=vclamp((qb-c)/qa,0.0f,1.0f);}}}
    V3 ptA=V3_AplusB(a.base,V3_ScaleByF(d1,sc)), ptB=V3_AplusB(b.base,V3_ScaleByF(d2,tc)), diff=V3_AsubB(ptA,ptB); distSq=V3_dot(diff,diff); if(distSq >= radSum * radSum) return r; float dist=vsqrtf(vmax(distSq,0.0f)); r.pen=radSum - dist; r.hit=true; r.normal=(dist < PHY_EPSILON) ? (V3){0,1,0} : V3_ScaleByF(diff,1.0f/dist); r.point = V3_AplusB(ptB,V3_ScaleByF(r.normal,b.rad)); return r;
}

INLINE Overlap SphBoxAxes(V3 ctr, float rad, V3 boxCtr, V3 ax, V3 ay, V3 az, V3 hExt) {
    Overlap r={0}; V3 d=V3_AsubB(ctr,boxCtr); float lx=V3_dot(d,ax), ly=V3_dot(d,ay), lz=V3_dot(d,az); V3 localClosest=V3_AplusB(V3_AplusB(V3_ScaleByF(ax,vclamp(lx,-hExt.x,hExt.x)), V3_ScaleByF(ay,vclamp(ly,-hExt.y,hExt.y))), V3_ScaleByF(az,vclamp(lz,-hExt.z,hExt.z))); V3 delta = V3_AsubB(d,localClosest); float distSq = V3_dot(delta,delta); if (distSq >= rad * rad) return r; r.hit=true; float dist = vsqrtf(vmax(distSq,0.0f));
    if(dist>PHY_EPSILON){r.normal=V3_ScaleByF(delta,1.0f/dist); r.pen=rad-dist;}else{float mind=hExt.x-vabs(lx),dy=hExt.y-vabs(ly),dz=hExt.z-vabs(lz); V3 nAx=V3_ScaleByF(ax,lx > 0.0f ? 1.0f : -1.0f); if(dy < mind){mind=dy; nAx=V3_ScaleByF(ay,ly > 0.0f ? 1.0f : -1.0f);} if(dz < mind){mind=dz; nAx=V3_ScaleByF(az, lz > 0.0f ? 1.0f : -1.0f);} r.normal=nAx; r.pen=rad+mind; } r.point=V3_AsubB(ctr, V3_ScaleByF(r.normal,rad-r.pen)); return r;
}
...
INLINE Overlap CapBox(ShapeCapsule c,ShapeBox b){V3 ax=quat_rot_v3(b.rot,(V3){1,0,0}),ay=quat_rot_v3(b.rot,(V3){0,1,0}),az=quat_rot_v3(b.rot,(V3){0,0,1}); V3 d=V3_AsubB(c.tip,c.base); Overlap best=SphBoxAxes(c.base,c.rad,b.ctr,ax,ay,az,b.hExt), r=SphBoxAxes(c.tip,c.rad,b.ctr,ax,ay,az,b.hExt); if(r.pen>best.pen){best=r;} if(V3_dot(d,d)>PHY_EPSILON*PHY_EPSILON){for(int k=1;k<8;k++){r=SphBoxAxes(V3_AplusB(c.base,V3_ScaleByF(d,k*.125f)),c.rad,b.ctr,ax,ay,az,b.hExt); if(r.pen>best.pen){best=r;}}} return best;}
static const u32 CollisionMaskTable[32] = {
    ...
    [13]=L_Default|L_Geometry|L_PlayerBullets|L_PhysObjects|L_Door|L_NPCBullet|L_Clip,/*L_Corpse*/ [14]=L_Default|L_TransparentFX|L_Geometry|L_NPC|L_PlayerBullets|L_Player|L_Corpse|L_PhysObjects|L_Door|L_InterDebris|L_NPCBullet|L_Clip,/*L_PhysObjects*/ [16]=L_Player|L_Player2,/*L_PlayerTriggerOnly*/
    ...
    [23]=L_Default|L_TransparentFX|L_Geometry|L_NPC|L_PlayerBullets|L_Player|L_PhysObjects|L_Trigger|L_NPCTrigger|L_Door|L_InterDebris|L_Player2|L_NPCBullet|L_NPCClip|L_Clip,/*L_NPCTrigger (Copy of L_NPC)*/ [24]=L_Default|L_TransparentFX|L_Geometry|L_PlayerBullets|L_Player|L_Corpse|L_PhysObjects|L_Door|L_InterDebris|L_Player2|L_Clip|L_CorpseSearchable,/*L_NPCBullet*/
    [26]=L_Player|L_Player2|L_NPC,/*L_Clip*/ [25]=L_Default|L_TransparentFX|L_Geometry|L_NPC|L_PlayerBullets|L_Player|L_PhysObjects|L_Trigger|L_NPCTrigger|L_Door|L_InterDebris|L_Player2|L_NPCBullet|L_NPCClip|L_Clip,/*L_NPCClip (Copy of L_NPC)*/ [29]=L_Default|L_PlayerBullets,/*L_CorpseSearchable*/ };
...
void ComputeConvexMeshInertiaTensor(u16 i) {
    ...
    for (u32 ti=0;ti<triCount;++ti) { // Accumulates without dividing by 6 for each saving that for the end for performance.
        u32 i0=modelTriangles[mi][ti*3+0], i1=modelTriangles[mi][ti*3+1], i2=modelTriangles[mi][ti*3+2]; V3 v0=MeshVert(mi,i0), v1=MeshVert(mi,i1), v2=MeshVert(mi,i2); float det=V3_dot(v0,V3_Cross(v1,v2)); volAcc += det; // Signed 6x tetrahedron volume via scalar triple product
        cm[0]+=det*(v0.x + v1.x + v2.x); cm[1] += det*(v0.y + v1.y + v2.y); cm[2] += det*(v0.z + v1.z + v2.z);
        acc[0]+=det*(v0.x*v0.x + v0.x*v1.x + v1.x*v1.x + v0.x*v2.x + v1.x*v2.x + v2.x*v2.x);/*Analytic polynomial integration over volume*/ acc[1] += det*(v0.y*v0.y + v0.y*v1.y + v1.y*v1.y + v0.y*v2.y + v1.y*v2.y + v2.y*v2.y); acc[2] += det*(v0.z*v0.z + v0.z*v1.z + v1.z*v1.z + v0.z*v2.z + v1.z*v2.z + v2.z*v2.z);/*(Mirtich / Eberly polyhedron mass properties technique)*/
        acc[3]+=det*(2.0f*(v0.x*v0.y+v1.x*v1.y+v2.x*v2.y)+v0.x*v1.y+v1.x*v0.y+v0.x*v2.y+v2.x*v0.y+v1.x*v2.y+v2.x*v1.y); acc[4]+=det*(2.0f*(v0.x*v0.z+v1.x*v1.z+v2.x*v2.z)+v0.x*v1.z+v1.x*v0.z+v0.x*v2.z+v2.x*v0.z+v1.x*v2.z+v2.x*v1.z); acc[5]+=det*(2.0f*(v0.y*v0.z+v1.y*v1.z+v2.y*v2.z)+v0.y*v1.z+v1.y*v0.z+v0.y*v2.z+v2.y*v0.z+v1.y*v2.z+v2.y*v1.z); /*Multiplier 2.0f handles the algebraic cross-term polynomial expansion*/
    }
    if(vabs(volAcc) < PHY_EPSILON){return;} float sx=World.scale[i].x,sy=World.scale[i].y,sz=World.scale[i].z; float sd=World.mass[i]/(volAcc * 10.0f/*6/60*/), so = World.mass[i] / (volAcc * 20.0f/*6/120*/);/*Diagonals=6/60, off-diagonals=6/120, since those have the 2.0f multiplied above from the x^2 expansion resulting in 2xy terms.*/ float cx=cm[0]/(4.0f*volAcc),cy=cm[1]/(4.0f*volAcc),cz=cm[2]/(4.0f*volAcc); float scx=cx*sx,scy=cy*sy,scz=cz*sz,m=World.mass[i];/*Center of mass denominator 24 reduced by 6/24 = 1/4*/
    float Ixx = sd*(acc[1]*sy*sy + acc[2]*sz*sz) - m*(scy*scy + scz*scz); float Iyy = sd*(acc[0]*sx*sx + acc[2]*sz*sz) - m*(scx*scx + scz*scz); float Izz = sd*(acc[0]*sx*sx + acc[1]*sy*sy) - m*(scx*scx + scy*scy); float Ixy = -(so*acc[3]*sx*sy - m*scx*scy); float Ixz = -(so*acc[4]*sx*sz - m*scx*scz); float Iyz = -(so*acc[5]*sy*sz - m*scy*scz); // Parallel Axis Theorem: shifts rotation center from origin (0,0,0) to center of mass
    float r = modelBounds[mi] * vmax(vmax(sx,sy),sz); float mn = 0.04f * m * r * r; Ixx = vmax(Ixx,mn); Iyy = vmax(Iyy,mn); Izz = vmax(Izz,mn); // Clamp diagonal inertia to a minimum floor of 10% of the I=2/5ths*mr^2=0.4mr^2 hence 0.04 of spherical inertia to avoid NaN's.
    ...
    iI[3]=(Ixz*Iyz - Ixy*Izz)*invDet; iI[4]=(Ixy*Iyz - Iyy*Ixz)*invDet; iI[5]=(Ixy*Ixz - Ixx*Iyz)*invDet;      /* [ 4  5  2 ]*/    World.invTnsrValid[i]=true;
}
...
INLINE Manifold BoxBox(ShapeBox a, ShapeBox b) {
    ...
    for (int i=0;i<3;i++)for(int j=0;j<3;j++){int i1=(i+1)%3,i2=(i+2)%3,j1=(j+1)%3,j2=(j+2)%3; float t=vabs(V3_dot(T,aAxes[i2])*R[i1][j]-V3_dot(T,aAxes[i1])*R[i2][j]),ra=aExt[i1]*AbsR[i2][j]+aExt[i2]*AbsR[i1][j],rb=bExt[j1]*AbsR[i][j2]+bExt[j2]*AbsR[i][j1]; if(t>ra+rb)return m; float axLenSq=1.f-(R[i][j]*R[i][j]); if(axLenSq>1e-4f){float ov=((ra+rb)-t)/vsqrtf(axLenSq); if(ov<minOverlap-MANIFOLD_TIE_MARGIN){V3 ea=V3_Cross(aAxes[i],bAxes[j]); minOverlap=ov; bestAxis=6+i*3+j; flipNormal=(V3_dot(T,ea)<0.f);}}}
    ...
    if(bestAxis < 3){refAxis=bestAxis; normal=flipNormal ? aAxes[refAxis] : V3_ScaleByF(aAxes[refAxis],-1.f); refAxes=aAxes; refExt=aExt; incAxes=bAxes; incExt=bExt; refCenter=a.ctr; refNormal=V3_ScaleByF(normal,-1.0f);/*Points A->B*/}else{refAxis=bestAxis-3; normal=flipNormal ? bAxes[refAxis] : V3_ScaleByF(bAxes[refAxis], -1.f); refAxes=bAxes; refExt=bExt; incAxes=aAxes; incExt=aExt; refCenter=b.ctr; refNormal=normal;}/*Points B->A*/
    m.normal=normal; refCenter=V3_AplusB(refCenter, V3_ScaleByF(refNormal, refExt[refAxis])); float minDot=1e9f, incSign=1.0f; int incAxis=0;/*Push refCenter exactly to the touching face*/ for (int i=0; i<3; ++i) { float dot = V3_dot(incAxes[i], refNormal); if (dot < minDot) { minDot = dot; incAxis = i; incSign = 1.0f; } if (-dot < minDot) { minDot = -dot; incAxis = i; incSign = -1.0f; } }/*Find incident face (most anti-parallel to reference normal)*/
    V3 incBoxCtr = (bestAxis < 3) ? b.ctr : a.ctr; V3 incCenter = V3_AplusB(incBoxCtr, V3_ScaleByF(incAxes[incAxis], incSign * incExt[incAxis])); int i1 = (incAxis+1)%3, i2 = (incAxis+2)%3; V3 ext1 = V3_ScaleByF(incAxes[i1], incExt[i1]); V3 ext2 = V3_ScaleByF(incAxes[i2], incExt[i2]);
    V3 clipped[8]; int clipCount = 4; clipped[0] = V3_AplusB(incCenter, V3_AplusB(ext1, ext2)); clipped[1] = V3_AsubB(incCenter, V3_AsubB(ext1, ext2)); clipped[2] = V3_AsubB(incCenter, V3_AplusB(ext1, ext2)); clipped[3] = V3_AplusB(incCenter, V3_AsubB(ext1, ext2)); int r1 = (refAxis+1)%3, r2 = (refAxis+2)%3;
    ...
    for (int i=0; i<clipCount; ++i) { float dist = V3_dot(refNormal, clipped[i]) - refPlaneDist; if (dist <= 0.001f) { bool isDup = false; for(int k=0; k<m.n; ++k) { V3 diff = V3_AsubB(clipped[i], m.p[k].point); if (V3_dot(diff, diff) < 0.00001f) { isDup = true; break; } } if (!isDup && m.n < MANIFOLD_MAX) { m.p[m.n].point=clipped[i]; float pen=-dist; if(pen < 0){pen=0;} m.p[m.n].pen=pen; if (pen > m.maxPen){m.maxPen=pen;} m.n++; } }}
...
V3 HullSupport(u16 m, const float* M, u16 adjIdx, V3 dWorld) {
    ...
    if (unlikely(!haveAdj || V3_dot(dLocal, dLocal) < 0.000001f || n <= 64)) {float bestDot=-3.402823466e38F; u32 bestIdx=0; for(u32 i=0;i<n;++i){float dot=p[i*3]*dLocal.x+p[i*3+1]*dLocal.y+p[i*3+2]*dLocal.z; if(dot>bestDot){bestDot=dot; bestIdx=i;}} return MvVert(M,(V3){p[bestIdx*3],p[bestIdx*3+1],p[bestIdx*3+2]});} u16 curr=cvxAdjStart[adjIdx]; if(curr >= n)curr=0; float currDot=p[curr*3]*dLocal.x+p[curr*3+1]*dLocal.y+p[curr*3+2]*dLocal.z;
    for(u32 steps=0;steps<n;++steps){/*Hill-climbing using adjacency information*/u16 next=curr; float nextDot=currDot; u32 s=cvxAdjOffsets[adjIdx][curr],e=cvxAdjOffsets[adjIdx][curr+1]; for(u32 i=s;i<e;++i){u16 nb=cvxAdjLists[adjIdx][i]; float d=p[nb*3]*dLocal.x+p[nb*3+1]*dLocal.y+p[nb*3+2]*dLocal.z; if(d>nextDot){nextDot=d; next=nb;}} if(next == curr)break; curr=next; currDot=nextDot;} return MvVert(M,(V3){p[curr*3],p[curr*3+1],p[curr*3+2]});
}
...
bool GJKNextSimplex(Simplex3D *s, V3 *dir) {
    V3 A=s->v[s->n-1],AO={-A.x,-A.y,-A.z}; V3 wAA=s->wA[s->n-1],wBA=s->wB[s->n-1]; if(s->n == 2){V3 AB=V3_AsubB(s->v[0],A); if(V3_dot(AB,AB)<PHY_EPSILON)AB=V3_AplusB(AB,V3_ScaleByF(*dir,0.001f)); if(V3_dot(AB,AO)>0.f){*dir=V3_Cross(V3_Cross(AB,AO),AB);}else{s->n=1; GJKSet(s,0,A,wAA,wBA); *dir=AO; } if(V3_dot(*dir,*dir)<PHY_EPSILON){V3 px=(vabs(AB.x)>0.9f) ? (V3){0,1,0} : (V3){1,0,0}; *dir=V3_Cross(AB,px);} return true;}
    if (s->n == 3) {
...
void FeatureOverlap(V3 sc, float sr, V3 pt, Overlap* r){V3 delta=V3_AsubB(sc,pt); float dist2=V3_dot(delta,delta); int hit=(dist2<sr*sr); float dist=vsqrtf(vmax(dist2,0.0f)); float nMask=(dist>PHY_EPSILON) ? 1.0f : 0.0f; float invD=1.0f/vmax(dist,PHY_EPSILON); V3 n=V3_AplusB(V3_ScaleByF(delta,invD*nMask),V3_ScaleByF((V3){0.0f,1.0f,0.0f},1.0f-nMask)); float pen=(sr-dist)*(float)hit; int better=(pen>r->pen); r->hit=r->hit|(hit&better); r->point=better ? pt : r->point; r->normal=better ? n : r->normal; r->pen=better ? pen : r->pen;}
void SphTriTest(V3 sc, float sr, u16 mesh, u32 ti, const float* mx, Overlap* r) {
    V3 a,b,c; MeshTri(mesh,ti,mx,&a,&b,&c); V3 ab=V3_AsubB(b,a), ac=V3_AsubB(c,a), ap=V3_AsubB(sc,a); float d1=V3_dot(ab,ap), d2=V3_dot(ac,ap); if(d1 <= 0.0f && d2 <= 0.0f){FeatureOverlap(sc,sr,a,r); return;} V3 bp=V3_AsubB(sc,b); float d3=V3_dot(ab,bp), d4=V3_dot(ac,bp); if(d3 >= 0.0f && d4 <= d3){FeatureOverlap(sc,sr,b,r); return;} V3 cp=V3_AsubB(sc,c); float d5=V3_dot(ab,cp), d6=V3_dot(ac,cp); if(d6>=0.f && d5<=d6){FeatureOverlap(sc,sr,c,r); return;}
    float vc=d1*d4-d3*d2; if(vc<=0.f&&d1>=0.f&&d3<=0.f){float v=d1/(d1-d3); V3 pt=V3_AplusB(a,V3_ScaleByF(ab,v)); FeatureOverlap(sc,sr,pt,r); return;} float vb=d5*d2-d1*d6; if(vb<=0.f && d2>=0.f && d6<=0.f){float w=d2/(d2-d6); V3 pt=V3_AplusB(a,V3_ScaleByF(ac,w)); FeatureOverlap(sc,sr,pt,r); return;} float va=d3*d6-d5*d4; if(va<=0.f&&(d4-d3)>=0.f && (d5-d6)>=0.f){float w=(d4-d3)/((d4-d3)+(d5-d6)); V3 bc=V3_AsubB(c,b); V3 pt=V3_AplusB(b,V3_ScaleByF(bc,w)); FeatureOverlap(sc,sr,pt,r); return;}
    V3 n = V3_Cross(ab,ac); float nLen=V3_Mag(n); if(nLen<PHY_EPSILON) return; n=V3_ScaleByF(n,1.f/nLen); float dist=V3_dot(n,ap), absDist=vabs(dist); if (absDist < sr) { V3 fn = (V3){-n.x,-n.y,-n.z};/*For some reason it always needs negated to work properly.*/ Overlap t={true,V3_AsubB(sc,V3_ScaleByF(fn,absDist)),fn,sr-absDist}; if(t.pen>r->pen) *r=t; }
}
...
void SeedEPA(EPAState *epa, const Simplex3D *s) {static const int kTetFaces[4][3] = {{0,1,2},{0,3,1},{0,2,3},{1,3,2}}; epa->nv = 0; epa->nf = 0; for (int i = 0; i < 4; i++) { epa->ev[epa->nv].wA = s->wA[i]; epa->ev[epa->nv].wB = s->wB[i]; epa->ev[epa->nv].v=s->v[i]; epa->nv++;} for(int f=0;f<4;++f){EPAFace face=MakeEPAFace(epa->ev,kTetFaces[f][0],kTetFaces[f][1],kTetFaces[f][2]); if(face.d>=0.f && epa->nf<EPA_MAX_FACES){epa->ef[epa->nf++]=face;}}}
bool ExpandEPA(EPAState *epa, V3 sup, V3 wA, V3 wB) {
    ...
    for(int f=0;f<epa->nf;++f){if(V3_dot(epa->ef[f].n,V3_AsubB(sup,epa->ev[epa->ef[f].a].v))>0.f){int fv[3]={epa->ef[f].a,epa->ef[f].b,epa->ef[f].c}; for(int e=0;e<3;e++){int ea=fv[e],eb=fv[(e+1)%3]; bool found=false; for(int k=0;k<ne;k++)if(edges[k][0]==eb&&edges[k][1]==ea){edges[k][0]=edges[--ne][0]; edges[k][1]=edges[ne][1]; found=true; break;} if(!found && ne < EPA_MAX_EDGES){edges[ne][0]=ea; edges[ne++][1]=eb;}}} else keep[nk++]=f;}
...
INLINE void BvhNodeWorldAABB(const BvhNode* node, const float* mx, V3* wMn, V3* wMx) {
    __m128 col0=_mm_loadu_ps(mx + 0); __m128 col1=_mm_loadu_ps(mx + 4); __m128 col2=_mm_loadu_ps(mx + 8); __m128 tr=_mm_loadu_ps(mx + 12);  __m128 mn_v=_mm_setr_ps(node->mn.x,node->mn.y,node->mn.z,0.0f); __m128 mx_v=_mm_setr_ps(node->mx.x,node->mx.y,node->mx.z,0.0f); __m128 lc=_mm_mul_ps(_mm_add_ps(mn_v,mx_v),_mm_set1_ps(0.5f));/*lc=(mn+mx)*0.5f*/ __m128 lh=_mm_mul_ps(((__m128)((__v4sf)(mx_v)-(__v4sf)(mn_v))),_mm_set1_ps(0.5f));/*lh=(mx-mn)*0.5f*/
    __m128 lc_x = __builtin_shufflevector(lc,lc,0,0,0,0); __m128 lc_y = __builtin_shufflevector(lc,lc,1,1,1,1); __m128 lc_z = __builtin_shufflevector(lc,lc,2,2,2,2);/*Replicate lc.x, lc.y, lc.z across vectors*/ __m128 wc = _mm_add_ps(_mm_add_ps(_mm_mul_ps(col0,lc_x), _mm_mul_ps(col1,lc_y)), _mm_add_ps(_mm_mul_ps(col2,lc_z),tr)); /*wc=col0 * lc_x + col1 * lc_y + col2 * lc_z + tr*/ __v4si sign_mask = (__v4si)_mm_set1_ps(-0.0f); __v4si inv_mask = ~sign_mask;
    __m128 abs_col0 = (__m128)((__v4si)col0 & inv_mask); __m128 abs_col1 = (__m128)((__v4si)col1 & inv_mask); __m128 abs_col2 = (__m128)((__v4si)col2 & inv_mask);/*Abs of matrix columns via &*/ __m128 lh_x = __builtin_shufflevector(lh,lh,0,0,0,0); __m128 lh_y = __builtin_shufflevector(lh,lh,1,1,1,1); __m128 lh_z = __builtin_shufflevector(lh,lh,2,2,2,2);/*Replicate lh components*/
    __m128 wh = _mm_add_ps(_mm_add_ps(_mm_mul_ps(abs_col0,lh_x),_mm_mul_ps(abs_col1,lh_y)),_mm_mul_ps(abs_col2,lh_z));/*wh = abs_col0 * lh_x + abs_col1 * lh_y + abs_col2 * lh_z*/ __m128 wMn_v=((__m128)((__v4sf)(wc) - (__v4sf)(wh))); __m128 wMx_v=_mm_add_ps(wc,wh);/*wMn = wc - wh, wMx = wc + wh*/ wMn->x = wMn_v[0]; wMn->y = wMn_v[1]; wMn->z = wMn_v[2]; wMx->x = wMx_v[0]; wMx->y = wMx_v[1]; wMx->z = wMx_v[2];
}
...
void CvxTriTest(CvxMshCtx* ctx, V3 ta, V3 tb, V3 tc) {
    u16 hullMesh=ctx->hullMesh; Manifold* best=&ctx->best; if (vmin(ta.x,vmin(tb.x,tc.x))>ctx->hb.mx.x || vmax(ta.x,vmax(tb.x,tc.x))<ctx->hb.mn.x || vmin(ta.y,vmin(tb.y,tc.y))>ctx->hb.mx.y || vmax(ta.y,vmax(tb.y,tc.y))<ctx->hb.mn.y || vmin(ta.z,vmin(tb.z,tc.z))>ctx->hb.mx.z || vmax(ta.z,vmax(tb.z,tc.z))<ctx->hb.mn.z){return;} V3 triEdge1=V3_AsubB(tb,ta), triEdge2=V3_AsubB(tc,ta); V3 triN=V3_Cross(triEdge1,triEdge2); float triLenSq=V3_dot(triN,triN);
    if (triLenSq < PHY_EPSILON){return;} triN = V3_ScaleByF(triN, 1.0f / vsqrtf(triLenSq)); if ((ctx->hullRadius - vabs(V3_dot(triN,V3_AsubB(ctx->hullCenter, ta)))) <= (best->n ? best->maxPen + MANIFOLD_TIE_MARGIN : 0.0f)){return;} SupportCtx supCtx = (SupportCtx){ctx->boxV ? _supA_boxShape : _supA_hull, _supB_tri, .meshA=hullMesh, .matA=ctx->hullMx, .adjA=ctx->adjHull, .adjB=ctx->adjHull, .ta=ta, .tb=tb, .tc=tc, .boxShape=ctx->boxShape};
    GJKResult gjk = RunGJK(&supCtx,GJK_ITER); if(!gjk.hit){return;} Simplex3D *s = &gjk.s;
    while (s->n<4) {
        V3 fallbackDir={0.0f,1.0f,0.0f}; if(s->n==1) fallbackDir=(vabs(s->v[0].x)>0.5f)?(V3){0.0f,1.0f,0.0f}:(V3){1.0f,0.0f,0.0f}; else if(s->n==2){V3 edge=V3_AsubB(s->v[1],s->v[0]); fallbackDir=V3_Cross(edge,(vabs(edge.x)>0.5f)?(V3){0.0f,1.0f,0.0f}:(V3){1.0f,0.0f,0.0f});} else if(s->n==3){V3 e1=V3_AsubB(s->v[1],s->v[0]), e2=V3_AsubB(s->v[2],s->v[0]); fallbackDir=V3_Cross(e1,e2);}
    ...
    for (int it=0;it<EPA_ITER;++it){int bf=-1; float bd=1e9f; for (int f=0;f<epa.nf;f++)if(epa.ef[f].d<bd){bd=epa.ef[f].d;bf=f;} if(bf<0)break; V3 bn=epa.ef[bf].n; V3 wA,wB; GetSupportPair(&supCtx,bn,&wA,&wB); V3 sup=V3_AsubB(wA,wB); if(V3_dot(bn,sup)-bd<PHY_EPSILON){ if(V3_dot(bn,triN)<0.0f){bn=triN;} tHit=true; tN=bn; tD=bd; tP=EPAContactPoint(epa.ev,epa.ef[bf].a,epa.ef[bf].b,epa.ef[bf].c); break;} if(!ExpandEPA(&epa,sup,wA,wB))break;} if (!tHit){return;} V3 deepPoint=tP;
    if (!best->n) { best->normal=tN; best->maxPen=tD; best->p[best->n++]=(ManifoldPt){deepPoint,tD}; ctx->bestTa=ta; ctx->bestTb=tb; ctx->bestTc=tc; ctx->bestTriN=tN; ctx->bestTriD=tD; ctx->bestDeepPoint=deepPoint; ctx->haveBestTri=true; }
    else {
...
Manifold CvxMsh(u16 hullMesh, const float* hullMx, u16 triMesh, const float* triMx, u16 adjHull) {
    Manifold z={0}; if(hullMesh>=MAX_MDLS||adjHull>=MAX_MDLS||triMesh>=MAX_MDLS)return z; u32 hn=modelVertexCounts[hullMesh]; if(!hn)return z; CvxMshCtx ctx={0}; ctx.hullMesh=hullMesh; ctx.hullMx=hullMx; ctx.adjHull=adjHull; AABB3 hb;
...
Manifold CvxCvx(u16 meshA, u16 meshB, const float* matA, const float* matB, u16 adjA, u16 adjB) {
    Manifold m={0}; if(meshA>=MAX_MDLS||adjA>=MAX_MDLS||meshB>=MAX_MDLS||adjB>=MAX_MDLS)return m; SupportCtx ctx = (SupportCtx){_supA_hull, _supB_hull, .meshA=meshA, .meshB=meshB, .matA=matA, .matB=matB, .adjA=adjA, .adjB=adjB}; GJKResult gjk = RunGJK(&ctx,GJK_ITER); if(!gjk.hit)return m; if(gjk.s.n<4) RunGJKFallback(&ctx,&gjk.s); if(gjk.s.n<4)return m; EPAState epa; SeedEPA(&epa,&gjk.s);
    for(int it=0;it<EPA_ITER;++it) {
        ...
        if (V3_dot(bn,sup)-bd<PHY_EPSILON) {
            ...
            if (nVertsB > 0) {
                ...
                for(u32 i=0;i<nVertsB;++i){const u8* p=vb+i*12; V3 ptLocal=*(V3*)p; V3 pt=MvVert(matB,ptLocal); float distToPlane=V3_dot(bn,pt)-planeDist; if(vabs(distToPlane)<thicknessTolerance){float ptPen=bd-distToPlane; if(ptPen>0.0f){bool isDup=false; for(int k=0;k<m.n;++k){V3 diff=V3_AsubB(pt,m.p[k].point); if(V3_dot(diff,diff)<0.00001f){isDup=true; break;}} if(!isDup&&m.n<MANIFOLD_MAX){m.p[m.n++]=(ManifoldPt){pt,ptPen};} if(m.n>=MANIFOLD_MAX){break;}}}}
            } return m;
    ...
    for (int it=0;it<SOLVER_ITER_GLOBAL;++it) {
        ...
        for (u32 c=0;c<gContactCount;++c) {
            ...
            for (int p=0;p<sc->m.n;++p) {
                u16 a = sc->a, b = sc->b; V3 n = sc->m.normal; V3 rAarm = sc->rA[p], rBarm = sc->rB[p]; float targetVn = sc->targetVn[p]; float *accumN = &sc->accumN[p], *accumT = &sc->accumT[p]; bool bStatic = sc->bStatic; float invMassA = sc->invMassA, invMassB = sc->invMassB, invSumN=sc->invSumN[p]; if (invSumN < PHY_EPSILON) continue;
                ...
                if (tLen > 0.0001f) {
                    ...
                    if (invSumT > PHY_EPSILON) {
                        ...
                        float maxT = friction * (*accumN);  float newAccumT = vclamp(*accumT + jt, -maxT, maxT); jt = newAccumT - *accumT; *accumT = newAccumT; float deltaVt = jt * invSumT; if (deltaVt < 0.0f) deltaVt = -deltaVt; if (deltaVt > maxDelta) maxDelta = deltaVt;
...
void PrepareSolverContact(u16 a, u16 b, const Manifold *m, float dt) {
    if (!m->n || (World.col[b] == COLTYPE_MSH && World.col[a] == COLTYPE_MSH)) return; if (gContactCount >= MAX_GLOBAL_CONTACTS) { WarnPhysicsCapacity(0,"Ran out of global contact slots!"); return; }
    ...
    if (a == PLAYER1 || b == PLAYER1 || IdxIsNPC(World.instances[a].index) || IdxIsNPC(World.instances[b].index)) bouncinessA = bouncinessB = 0.0f;/*a/b are instance slots; IdxIsNPC wants the const index, so this used to only match bodies that happened to sit in slots 419-447*/
    for (int i=0;i<m->n;++i) {
...
void Physics(float dt) {
    ...
    for (u8 s=0;s<World.substeps;++s) {
        ...
        for (u16 i=0;i<World.instCount;++i) { // 0. Broadphase cell lists
            ...
            u32 cell=(u32)World.instances[i].cellIndex; if(cell < WORLDX*WORLDX){if(cellCounts[cell]<128){cellLists[cell][cellCounts[cell]++]=i;}else{WarnPhysicsCapacity(2,"Reached the 128 bodies-per-cell limit; additional bodies in this cell are skipped.");}} u16 idx=World.instances[i].index; bool isTrigger=((idx>=595&&idx<=601)||idx==703||idx==706||idx==746); if(isTrigger&&(World.instances[i].entflags&EF_ACTIVE)){if(numTriggers<128){triggerVolumes[numTriggers++]=i;}else{WarnPhysicsCapacity(3,"Reached the 128 trigger-volume limit; additional triggers are skipped.");}}
        }
        gContactCount=0;
            ...
            for (i32 dx = -radCells; dx <= radCells; ++dx) {
                for (i32 dz = -radCells; dz <= radCells; ++dz) { // 2. Collisions
                    ...
                    for (u16 k = 0; k < cellCounts[cell]; ++k) {
                        ...
                        if (unlikely(Cheats.noclip && b == PLAYER1)) continue; if (World.instances[b].index==703 || !(mask & World.layer[b]) || World.col[b] == COLTYPE_NONE) continue; if (unlikely((World.instances[b].entflags & EF_RIGIDBODY) && !World.physSleep[b] && b > a)) continue; // Prevent doubled restitutions; asleep b handled as static collider
                        ...
                        if (mf.n) { if (IsImpactGrenade(World.instances[a].index)) grenadeImpactQueued[a]=1; if ((World.instances[b].entflags & EF_ACTIVE) && IsImpactGrenade(World.instances[b].index)) grenadeImpactQueued[b]=1; V3 impactPoint=mf.p[0].point; if(IsImpactProjectile(World.instances[a].index))QueueProjectileImpact(a,b,impactPoint,mf.normal);if(IsImpactProjectile(World.instances[b].index))QueueProjectileImpact(b,a,impactPoint,V3_ScaleByF(mf.normal,-1.0f)); if (likely(contactCount < 32)) { contactsMani[contactCount] = mf; contactsOther[contactCount] = b; contactCount++; }else{WarnPhysicsCapacity(4,"Reached the 32 candidate-manifolds-per-body limit; additional contacts are skipped.");} }
                    }
                }
            }
        ...
        for (u16 i=0;i<numTriggers;++i) {
            ...
            for (u16 o=0;o<dynamicEntityCount;++o) { // 4. Triggers
                ...
                bool touches = false; 
                if(trigdx==703){touches=(other==PLAYER1)&&CapCap(Entity_GetCap(other),teleportCap).hit;}
                else if(World.col[other] == COLTYPE_CAP){ShapeCapsule oc=Entity_GetCap(other); V3 capMid=V3_ScaleByF(V3_AplusB(oc.tip,oc.base),0.5f); touches=CapsuleTouchesOBB(oc.base,oc.rad,trigBox) || CapsuleTouchesOBB(capMid,oc.rad,trigBox) || CapsuleTouchesOBB(oc.tip,oc.rad,trigBox);}
                else {float sphereRadius=vmax(vmax(vabs(World.colliderSize[other].x*World.scale[other].x),vabs(World.colliderSize[other].y*World.scale[other].y)),vabs(World.colliderSize[other].z*World.scale[other].z)); if(sphereRadius<=0.0f)sphereRadius=0.32f; touches=CapsuleTouchesOBB(World.position[other],sphereRadius,trigBox);}
                if (!touches) continue; if(trigdx==596){u16 mark=(u16)(self+1);bool entering=gravityLiftWasInside[other]!=mark&&gravityLiftTouched[other]!=mark;gravityLiftTouched[other]=mark;trigger_gravitylift_touch(self,other,entering);continue;} if(trigdx==703){u16 mark=(u16)(self+1);if(teleportWasTouching!=mark&&teleportTouched!=mark)TeleportTouchOnTriggerEnter(self,other);teleportTouched=mark;continue;}
                switch(trigdx) {
                    case 554/*prop_cyber_exit*/:if(other == PLAYER1){UIExitCyberspace();} break;   case 595/*trigger_cyberpush*/:if(other == PLAYER1 && World.diffCyb >= 1){AddForce(other,V3_ScaleByF(World.instances[self].direction,World.instances[self].force*World.mass[other]),false); World.Sys_Music.cyberTube=true;} break;
                    ...
                    case 706/*info_elev_destination*/:if(other == PLAYER1){World.Sys_Music.inZone=true; World.Sys_Music.elevator=true;} break; /* elevator shaft: override level music, same path as trigger_music/TT_Elev */
                    case 746/*weapon_grenadeenergmine_live*/:if(other == PLAYER1){TakeEnergy(256.0f);} break;
                }
            }
        }
        if(!gravityLiftTouched[PLAYER1])GravityLiftApplyTopSupport(PLAYER1);
        ladderWalkOff = ladderTouched && (World.position[PLAYER1].y > ladderTopY + 0.48f); if (!ladderTouched) World.invP1.ladderState=0; mcpy(gravityLiftWasInside,gravityLiftTouched,sizeof(gravityLiftWasInside)); teleportWasTouching=teleportTouched;
    }
            ...
            for (i32 dx=-WAKE_CELLS; dx<=WAKE_CELLS && !nearAwake; ++dx)
              for (i32 dz=-WAKE_CELLS; dz<=WAKE_CELLS && !nearAwake; ++dz) {
                ...
                for (u16 k=0;k<cellCounts[cl];++k) {
                    u16 j = cellLists[cl][k]; if (j==i) continue; u32 ej = World.instances[j].entflags; if (World.physSleep[j] || !(ej & EF_ACTIVE)) continue;/*asleep/inactive bodies don't wake others*/ float sj2 = V3_dot(World.velocity[j], World.velocity[j]); bool jMoving = sj2 > 0.0025f; bool jAnimWaking = AnimWaking(j); if (!(jMoving || jAnimWaking)) continue; /*not a waker -> lets i sleep*/
...
void ApplyPlayerMovements(float dt) {
    ...
    bool skatesAct = boosterOn && World.invP1.hwVersSetting[HW_BST_IDX]==0, boostAct = boosterOn && !skatesAct && World.invP1.ladderState==0;/*BoosterSetToSkates()/BoosterSetToBoost() split the one booster into two item-tab modes; skates keep normal sprinting, boost mode is ignored outright on a ladder*/
    if(onLadder&&!grounded&&!jumpjettin&&World.velocity[PLAYER1].y>0.65f&&ladderSFXFinished<World.pauseRelativeTime){play_wav(sounds[137],AppliedFXVol(0.2f),World.position[PLAYER1],true);ladderSFXFinished=World.pauseRelativeTime+1.0;}
    if (JumpDown() && (grounded || inGravLift || onLadder) && !jumpjettin) {
        ...
        else{if(!Cheats.noclip){World.velocity[PLAYER1].y+=(World.invP1.fatigue>80.0f ? 2.0f : 4.51f)+0.2f; if(!boostAct&&!Cheats.noclip){World.invP1.fatigue += 6.5f;}} RaycastHit jhit=Raycast(World.position[PLAYER1],(V3){0.0f,-1.0f,0.0f},2.0f,LMASK_PLAYER_FEET); FootStepType jfstp=jhit.hit ? GetFootstepTypeForPrefab(World.instances[jhit.hitInstanceIndex].index) : FSTP_Concrete; float jumpVol=World.invP1.fatigue>80.0f ? 0.5f : 1.0f; play_wav(JumpSound(jfstp),AppliedFXVol(jumpVol),World.position[PLAYER1],true);}
    }
    if (Jump() && jumpjettin && World.invP1.energy > 0.0f) {
        if (World.invP1.doubleJumpFinished > World.pauseRelativeTime) { /*Jump-jet lunge: any second press inside the double-click window, grounded or not, bursts forward instead of up*/
            World.invP1.doubleJumpFinished = 0.0; if (!Cheats.noclip) {World.velocity[PLAYER1] = V3_AplusB(World.velocity[PLAYER1], V3_ScaleByF(p->forward, 1.5f * (4.51f + 0.2f)));}/*1.5x the standard jump's full upward impulse, unfatigued: the lunge is paid for in energy, not fatigue*/ TakeEnergy(15.0f); World.invP1.jumpJetFinished = World.pauseRelativeTime + 0.1f; }
        else {
        if (!Cheats.noclip) {World.velocity[PLAYER1].y += 1.3f;} World.invP1.jumpJetFinished = World.pauseRelativeTime + 0.1f; World.invP1.doubleJumpFinished = World.pauseRelativeTime + 0.5f; if (World.invP1.jumpJetSuckFinished < World.pauseRelativeTime) { World.invP1.jumpJetSuckFinished=World.pauseRelativeTime+1.0f; float energysuck = 11.0f; switch (World.invP1.hwVersSetting[10]) { case 0: energysuck=11.0f; break; case 1:energysuck=26.0f; break; case 2:energysuck=22.0f; break; } TakeEnergy(energysuck); } } }
    float lastRatio = World.invP1.currentCrouchRatio; World.invP1.currentCrouchRatio = smooth_damp(lastRatio,targR,&World.invP1.crouchingVelocity,transitionSec,dt);
    if (World.invP1.currentCrouchRatio >= 1.0f) { World.invP1.currentCrouchRatio = 1.0f; if(p->bodyState == BodyState_StandingUp){p->bodyState=BodyState_Standing;} } else if (p->bodyState == BodyState_CrouchingDown && World.invP1.currentCrouchRatio <= PLAYER_CROUCH_RATIO) { World.invP1.currentCrouchRatio = PLAYER_CROUCH_RATIO; p->bodyState = BodyState_Crouch; }
    else if (p->bodyState == BodyState_ProningUp && World.invP1.currentCrouchRatio >= PLAYER_CROUCH_RATIO) { World.invP1.currentCrouchRatio = PLAYER_CROUCH_RATIO; p->bodyState = BodyState_Crouch; } else if (p->bodyState == BodyState_ProningDown && World.invP1.currentCrouchRatio <= PLAYER_PRONE_RATIO) { World.invP1.currentCrouchRatio = PLAYER_PRONE_RATIO; p->bodyState = BodyState_Prone; }
    ...
    bool isSprinting=Sprint() && (grounded || inGravLift || World.invP1.ladderState>0 || Cheats.noclip); if(World.invP1.fatigueMoveFinished<World.pauseRelativeTime && (vabs(h)>0.0f || vabs(s)>0.0f) && grounded && (V3_dot(World.velocity[PLAYER1],World.velocity[PLAYER1])>0.1f && !Cheats.noclip)&&!boostAct){World.invP1.fatigue+=isSprinting ? 2.85f : 1.0f; World.invP1.fatigueMoveFinished=World.pauseRelativeTime+0.298f;/*Ensure no sync w/ bleedoff*/}
    if(World.invP1.footstepFinished < World.pauseRelativeTime && (vabs(h) > 0.0f || vabs(s) > 0.0f) && grounded && (PlayerIsMoving() && !Cheats.noclip) && !boostAct){
        RaycastHit fstep=Raycast(World.position[PLAYER1],(V3){0.0f,-1.0f,0.0f},2.0f,LMASK_PLAYER_FEET); play_wav(FootStepSound(fstep.hit ? GetFootstepTypeForPrefab(World.instances[fstep.hitInstanceIndex].index) : FSTP_Concrete),AppliedFXVol(random_range(0.4f,0.55f)),World.position[PLAYER1],true); World.invP1.footstepFinished = World.pauseRelativeTime + (isSprinting ? random_range(0.2f,0.3f) : random_range(0.35f,0.65f));
    }
    if(World.invP1.rustleFinished<World.pauseRelativeTime && (vabs(h)>0.0f || vabs(s)>0.0f) && (V3_dot(World.velocity[PLAYER1],World.velocity[PLAYER1])>0.1f && !Cheats.noclip) && PlayerIsMoving() && !boostAct){play_wav(sounds[random_range_u32(459,465)],AppliedFXVol(random_range(0.3f,0.5f)),World.position[PLAYER1],true); World.invP1.rustleFinished=World.pauseRelativeTime+(isSprinting ? random_range(0.4f,0.6f) : random_range(0.8f,1.2f));}
    ...
    if((isSprinting||boostAct) && isRunning){v=World.invP1.fatigue > 80.0f && !boostAct ? 10.0f : 17.0f;/*Boost mode reaches maxSprintSpeed on its own and skips the fatigue penalty; skates leave sprinting alone. The +1.2 is added exactly once below, as retval+bonus*/ if (b==BodyState_Standing||b==BodyState_Crouch||b==BodyState_CrouchingDown) v-=(WALK_SPEED-CROUCH_SPEED)*1.5f; else if(b==BodyState_Prone||b==BodyState_ProningDown||b==BodyState_ProningUp) v-=(WALK_SPEED-PLAYER_MAX_PRONE_SPEED)*2.f;}
    float speed = (setSpeedAdjusted ? speedAdjust : (skatesAct ? v*SKATE_SPEED : v) + (boostAct ? PLAYER_BOOSTER_SPEED_BOOST : 0.0f)) + (World.invP1.staminupActive ? 1.0f : 0.0f), accel=World.curLev!=LEVEL_CYBERSPACE ? (skatesAct ? WALK_SPEED_ACCEL/SKATE_SLIDE : (boostAct ? BOOST_ACCEL : WALK_SPEED_ACCEL)) : WALK_SPEED_ACCEL;/*SKATE trades accel for slide and pays it back as SKATE_SPEED so it never ends up slower than walking; BOOST adds the steady +1.2 forward push once, as retval+bonus*/ V3 targetVel = V3_ScaleByF(w,speed); 
    if (onLadder && !ladderWalkOff) { float climbSpeed = (isSprinting && isRunning) ? 3.0f : 1.3f; targetVel = (V3){p->right.x * s * speed * 0.3f, h * climbSpeed, p->right.z * s * speed * 0.3f}; accel = 5.0f; } else { if (vabs(vertInput) < 0.001f) { targetVel.y = World.velocity[PLAYER1].y; } }
    V3 dv = V3_AsubB(targetVel, World.velocity[PLAYER1]); dv = (V3){ vclamp(dv.x, -10.0f,10.0f), vclamp(dv.y, -10.0f,10.0f), vclamp(dv.z,-10.0f,10.0f) }; World.velocity[PLAYER1] = V3_AplusB(World.velocity[PLAYER1],V3_ScaleByF(dv,accel*vclamp(dt,0.0005f,0.1f)));
    ...
    if (grounded && !World.invP1.wasGrounded) {
        ...
        if(velChange>2.0f && World.invP1.noiseFinished<World.pauseRelativeTime){RaycastHit lhit=Raycast(World.position[PLAYER1],(V3){0.0f,-1.0f,0.0f},2.0f,LMASK_PLAYER_FEET); FootStepType lstp=lhit.hit ? GetFootstepTypeForPrefab(World.instances[lhit.hitInstanceIndex].index) : FSTP_Concrete; play_wav(JumpLandSound(lstp),AppliedFXVol(vclamp(velChange/11.72f,0.5f,1.0f)),World.position[PLAYER1],true); World.invP1.noiseFinished=World.pauseRelativeTime+0.2f;}
