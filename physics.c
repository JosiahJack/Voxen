...
void ApplyPlayerMovements(float dt) {
    ...
    if(World.invP1.rustleFinished<World.pauseRelativeTime && (vabs(h)>0.0f || vabs(s)>0.0f) && (V3_dot(World.velocity[PLAYER1],World.velocity[PLAYER1])>0.1f && !Cheats.noclip) && !World.boosterActive){play_wav(sounds[random_range_u32(459,465)],AppliedFXVol(random_range(0.3f,0.5f)),World.position[PLAYER1],true); World.invP1.rustleFinished=World.pauseRelativeTime+(isSprinting ? random_range(0.4f,0.6f) : random_range(0.8f,1.2f));}
