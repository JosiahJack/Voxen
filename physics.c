...
#include "common.h"
u16 cellLists[WORLDX*WORLDX][128],cellCounts[WORLDX*WORLDX]; u16 triggerVolumes[128]; u16 numTriggers; extern double game_actual_start_time; static bool ladderWalkOff = false; static float ladderTopY = 0.0f; void DrawBoxColliderColored(u16 i, Color col);
...
// TeleportTouch
void TeleportTouchOnTriggerEnter(u16 self, u16 o){Entity *e=&World.instances[self],*player=&World.instances[PLAYER1]; if(!e->touchEnabled||o!=PLAYER1){return;} if(player->health<=0.0f||e->justUsed>=World.pauseRelativeTime){return;} u16 dest=e->targetDestinationID<8 ? World.TeleportTouch_allTeleportTouches[e->targetDestinationID] : U16_MAX; if(dest==U16_MAX){return;} World.position[PLAYER1]=World.position[dest]; World.instances[dest].justUsed=World.pauseRelativeTime+1.0; play_wav(sounds[106],1.0f,World.position[dest],false);}
