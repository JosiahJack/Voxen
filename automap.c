...
INLINE int amNavVer(void) {/*nav hw ver: 0=none,1..3=v1..v3; gates zoom/overlays/cadence*/ return 3;/*TODO remove the testing version to use actual version: World.invP1.hwVers[HW_NAV_IDX];*/ }
...
INLINE void amPutPx(int x,int y,u8 r,u8 g,u8 b) {if((u32)x>=(u32)AM_W || (u32)y>=(u32)AM_H){return;} u8* p=&amPx[((u32)y*AM_W+(u32)x)*4]; p[0]=r; p[1]=g; p[2]=b; p[3]=255; }
static void amFillRect(int x0,int y0,int x1,int y1,u8 r,u8 g,u8 b) {/*x1,y1 exclusive*/if(x0<0){x0=0;} if(y0<0){y0=0;} if(x1>AM_W){x1=AM_W;} if(y1>AM_H){y1=AM_H;} for (int y=y0;y<y1;++y) for (int x=x0;x<x1;++x) { u8* p=&amPx[((u32)y*AM_W+(u32)x)*4]; p[0]=r; p[1]=g; p[2]=b; p[3]=255; } }
...
static void amLine(float ax,float ay,float bx,float by,u8 r,u8 g,u8 b,int w) { float dx=bx-ax,dy=by-ay,len=vsqrtf(dx*dx+dy*dy); int n=(int)vceil(len); if(n<1){n=1;} int o0=-(w/2),o1=o0+w-1; for (int i=0;i<=n;++i) { float t=(float)i/(float)n; int x=(int)vround(ax+dx*t),y=(int)vround(ay+dy*t); for (int oy=o0;oy<=o1;++oy) for (int ox=o0;ox<=o1;++ox) amPutPx(x+ox,y+oy,r,g,b); }}
...
static void amRaster(void) {
    ...
    for (u32 i=INSTS_1ST_IDX;i<World.instCount;++i) {/*Get floor heights for difference check to draw dark green line between cells of different heights.*/
        Entity* e=&World.instances[i]; if(!IdxIsGeometry(e->index) || !(e->entflags&EF_ACTIVE) || e->modelIndex>=MAX_MDLS){continue;} u16 m=e->modelIndex; u16 cm=amCardModel(m); if(!amComputeCard(cm)){continue;} V3 n=quat_rot_v3(World.rotation[i],(V3){amCardNx[cm],amCardNy[cm],amCardNz[cm]}); if(vabs(n.y)<0.7f){continue;}/*not horizontal*/
    ...
    for (u32 i=INSTS_1ST_IDX;i<World.instCount;++i) {/*wedge inst per cell*/Entity* e=&World.instances[i]; if(!IdxIsGeometry(e->index) || !(e->entflags&EF_ACTIVE) || e->modelIndex>=MAX_MDLS){continue;} u16 m=e->modelIndex; if(!amComputeModel(m) || !amMdlWall[m] || amHullN[m]!=3){continue;} int cx=PosGetCellCoordX(World.position[i].x),cz=PosGetCellCoordZ(World.position[i].z); if(cx>=0&&cx<64&&cz>=0&&cz<64){amWedgeInst[(u32)cz*64+(u32)cx]=(u16)i;}}
    for (u32 i=INSTS_1ST_IDX;i<World.instCount;++i) {/*diag-wall cells (card-based). diag cells skip closed edges; neighbors draw backs*/
        Entity* e=&World.instances[i]; if(!IdxIsGeometry(e->index) || !(e->entflags&EF_ACTIVE) || e->modelIndex>=MAX_MDLS){continue;} u16 m=e->modelIndex; u16 cm=amCardModel(m); if(!amComputeCard(cm)){continue;} V3 wn=quat_rot_v3(World.rotation[i],(V3){amCardNx[cm],amCardNy[cm],amCardNz[cm]}); if(vabs(wn.y)>0.7f){continue;}/*horizontal: no wall*/ if(vabs(wn.x)<0.3f || vabs(wn.z)<0.3f){continue;}/*axis-aligned, not diagonal*/
    ...
    for (int cz=0;cz<64;++cz) for (int cx=0;cx<64;++cx) {/*walls from grid closed edges (light green). grid-implicit; hulls miss them. diag cells skip closed edges. door-coincident edges skipped*/
        ...
        if(rx1<0.0f||rx0>(float)AM_W||ry1<0.0f||ry0>(float)AM_H){continue;}/*outside zoom window*/ float ewx0=x0c+(float)cx*CELLSZ,ewx1=ewx0+CELLSZ; float ewz0=z0c+(float)cz*CELLSZ,ewz1=ewz0+CELLSZ; if(st&CELL_CLOSEDNORTH){ if(!amEdgeHasDoor(1,ewz1,ewx0,ewx1)){amLine(rx0,ry0,rx1,ry0,0,220,0,2);} } if(st&CELL_CLOSEDSOUTH){ if(!amEdgeHasDoor(1,ewz0,ewx0,ewx1)){amLine(rx0,ry1,rx1,ry1,0,220,0,2);} }
    ...
    for (u32 i=INSTS_1ST_IDX;i<World.instCount;++i) {/*diag walls: vertical cards with X+Z normal (non-axis). longest XZ span drawn. chunks use 178 card*/
        Entity* e=&World.instances[i]; if(!IdxIsGeometry(e->index) || !(e->entflags&EF_ACTIVE) || e->modelIndex>=MAX_MDLS){continue;} u16 m=e->modelIndex; u16 cm=amCardModel(m); if(!amComputeCard(cm)){continue;} /*world normal via instance rot*/ V3 wn=quat_rot_v3(World.rotation[i],(V3){amCardNx[cm],amCardNy[cm],amCardNz[cm]}); /*diag wall check: vertical, non-axis*/ if(vabs(wn.y)>0.7f){continue;}/*horizontal: no wall*/
    ...
    for (u32 i=INSTS_1ST_IDX;i<World.instCount;++i) {/*hull outlines: dark green open steps only. closed walls drawn from grid*/
        Entity* e=&World.instances[i]; if(!IdxIsGeometry(e->index) || !(e->entflags&EF_ACTIVE) || e->modelIndex>=MAX_MDLS){continue;} u16 m=e->modelIndex; if(!amComputeModel(m) || !amMdlWall[m]){continue;} u8 hn=amHullN[m]; if(hn<3){continue;} float px[AM_MAXHULL],pz[AM_MAXHULL]; V3 ip=World.position[i]; Quaternion iq=World.rotation[i]; V3 is=World.scale[i];
    ...
    for (u32 i=INSTS_1ST_IDX;i<World.instCount;++i) {/*doors: closed=yellow span; open=yellow center ticks. bulkheads (499/509) + big elevator doors (508/510/511): end ticks only, 2 per cell. span=width axis, clamp 1 cell (2.56), 2-cell doors 2 cells (5.12). no card geo*/
        ...
        dx/=len; dy/=len; float nx=-dy,ny=dx;/*span dir + perp*/ float cx=(ax+bx)*0.5f,cy=(ay+by)*0.5f; if(!bulkhead){amLine(cx-dx*1.0f,cy-dy*1.0f,cx+dx*1.0f,cy+dy*1.0f,255,255,0,2);}/*3px center*/ amLine(ax-nx*1.0f,ay-ny*1.0f,ax+nx*1.0f,ay+ny*1.0f,255,255,0,2);/*end ticks*/ amLine(bx-nx*1.0f,by-ny*1.0f,bx+nx*1.0f,by+ny*1.0f,255,255,0,2); if(bulkhead){amLine(cx-nx*1.0f,cy-ny*1.0f,cx+nx*1.0f,cy+ny*1.0f,255,255,0,2);}/*mid tick*/
    }
    for (u32 i=INSTS_1ST_IDX;i<World.instCount;++i) {/*overlays (nav-gated): NPC dots, red cameras/CPU nodes*/
        Entity* e=&World.instances[i]; if(!(e->entflags&EF_ACTIVE) || e->health<=0.0f){continue;} int ci=e->index; float x=amWX(World.position[i].x),y=amWZ(World.position[i].z); if(x<-12.0f||y<-12.0f||x>AM_W+12.0f||y>AM_H+12.0f){continue;} int ecx=PosGetCellCoordX(World.position[i].x),ecz=PosGetCellCoordZ(World.position[i].z); if(!expl[(u32)ecz*64+(u32)ecx]){continue;} 
        if(IdxIsNPC(ci)){NPCType t=npcTable[ci-419].type; if(nav<amNPCNeedVer(t)){continue;} u8 r,g,b; amNPCColor(t,&r,&g,&b); amFillRect((int)(x-5.0f),(int)(y-5.0f),(int)(x+5.0f),(int)(y+5.0f),r,g,b);}else if(ci==477){amFillRect((int)(x-1.0f),(int)(y-1.0f),(int)(x+1.0f),(int)(y+1.0f),255,0,0);}else if(ci==478||ci==479){amFillRect((int)(x-5.0f),(int)(y-5.0f),(int)(x+5.0f),(int)(y+5.0f),255,0,0);}
    }
    {V3 p=World.position[PLAYER1],f=World.instances[PLAYER1].forward; float x=amWX(p.x),y=amWZ(p.z); float fl=vsqrtf(f.x*f.x+f.z*f.z); float dx=0.0f,dy=-1.0f; if(fl>1e-4f){ dx=f.x/fl; dy=-f.z/fl; }/*screen: +x right, -z up*/ float nx=-dy,ny=dx; amFillTri(x+dx*8.0f,y+dy*8.0f, x-dx*5.0f+nx*5.0f,y-dy*5.0f+ny*5.0f, x-dx*5.0f-nx*5.0f,y-dy*5.0f-ny*5.0f, 255,32,32);}
