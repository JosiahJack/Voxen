...
void InitFontAtlasses(void){
    ...
    TempFree(pc.pack_info);_make_gl_atlas(&fontAtlasTex,bmp);
    ...
    TempFree(pc2.pack_info);_make_gl_atlas(&fontAtlasTexStopD,bmp);
...
void RenderFormattedText(i16 x,i16 y,u32 color,u8 fontID,float scaleInput,const char* restrict format,...){
    ...
    glUseProgram(Sys_Render.textShaderProgram);glUniformMatrix4fv(0,1,GL_FALSE,uiOrthoProjection);
