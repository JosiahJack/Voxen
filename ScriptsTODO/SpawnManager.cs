	...
	void Start() {
		...
		if (World.diffCbt == 1) { numberToSpawn=(int) vfloor(numberToSpawn*0.5f); if (numberToSpawn < 1){numberToSpawn=1;} }
		if (World.diffCbt == 3) { numberToSpawn=(int) vfloor(numberToSpawn*1.5f); if (numberToSpawn < 1){numberToSpawn=1;} }
		if (World.diffCbt  > 3) { numberToSpawn=(int) vfloor(numberToSpawn*5f); /*Hehe :)*/}
    }

	public void Activate(bool alertEnemies) { alertEnemiesOnAwake = alertEnemies; active = true; delayFinished = World.pauseRelativeTime; }
	void Update() {
		if (World.paused || World.menuActive || !active) return;
		...
		delayFinished = World.pauseRelativeTime + random_range(minDelayBetweenSpawns,maxDelayBetweenSpawns);
		Spawn(index); // spawn then wait randomized amount of time
		count++;
		if (count >= numberToSpawn) { delayFinished = World.pauseRelativeTime + allSpawnedResetDelay; }
	}
	...
	void Spawn(int index) {
        ...
        World.position[instGO] = spot;
        if (!alertEnemiesOnAwake) { if (aic.index != 14) aic.wandering = true; return; }
	...
	bool AreaHidden(V3 spot) {
		...
		RaycastHit tempHit;
		if (Raycast(spot,ray,out tempHit,50.0f,mask)) { if (tempHit.collider.CompareTag("Player")) {return false;} }
		return true;
	}
}
