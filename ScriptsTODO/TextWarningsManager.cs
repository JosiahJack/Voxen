...
public class TextWarningsManager : MonoBehaviour {
	...
	void Start () {
		...
		for (int i=0;i<warningTextGObjects.Length;i++) {
			...
			initialized[i] = false;
			finishedTime[i] = World->pauseRelativeTime;
			uniqueID[i] = -1;
		}
	}
	...
	public void SendWarning(string message, float lifetime, int forcedReference, HUDColor col, int id) {
		...
		for (int i=setIndex;i>=0;i--) {
			...
			if (finishedTime[i] < World->pauseRelativeTime) { setIndex = i; break;}
		}
		...
		finishedTime[setIndex] = World->pauseRelativeTime + warningLifeTimes[setIndex];
	}
	...
	void Update() {
		...
		for (int i=0;i<warningTextGObjects.Length;i++) {
			if (!sEmpty(warningTexts[i].text)) {
				if (finishedTime[i] < World->pauseRelativeTime) {
