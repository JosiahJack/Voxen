 ...
 */
	...
	void Start () {
		...
		if (data_parser_isspace(resourceFolder)) resourceFolder = glowResourceFolder;
		if (data_parser_isspace(resourceFolder)) return;
	...
	void Update() {
		if (!Sys_Global.gamePaused && !Sys_Global.menuActive) {
			if (data_parser_isspace(resourceFolder)) { this.enabled = false; return; }
