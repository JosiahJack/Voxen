...
typedef enum{UI_ID_NONE,
    ...
    /*Center MFD*/UI_ID_CMFD_ADD_TO_INVENTORY,UI_ID_CMFD_TAB_MAIN,UI_ID_CMFD_TAB_HARDWARE,UI_ID_CMFD_TAB_GENERAL,UI_ID_CMFD_TAB_SOFTWARE,UI_ID_CMFD_PANEL,UI_ID_CMFD_WEAPON_HEADER,UI_ID_CMFD_SHOTS_HEADER,UI_ID_CMFD_WEAPON_ROW_0,UI_ID_CMFD_WEAPON_ROW_6=UI_ID_CMFD_WEAPON_ROW_0+6,UI_ID_CMFD_GREN_HEADER,UI_ID_CMFD_PATCH_HEADER,UI_ID_CMFD_GREN_USE_0,UI_ID_CMFD_GREN_USE_6=UI_ID_CMFD_GREN_USE_0+6,UI_ID_CMFD_GREN_ROW_0,UI_ID_CMFD_GREN_ROW_6=UI_ID_CMFD_GREN_ROW_0+6,
    ...
    /*Misc overlays*/UI_ID_CMFD_MISSION_TIMER,UI_ID_CMFD_CYBER_TIMER,UI_ID_CMFD_BIOMONITOR,UI_ID_CMFD_EDIT_PANEL,UI_ID_CMFD_EDIT_ROW_0,UI_ID_CMFD_EDIT_ROW_14=UI_ID_CMFD_EDIT_ROW_0+14,UI_ID_VMAIL_VIEWER,
...
INLINE void UIR(u32 id, i16 x, i16 y, i16 w, i16 h) { if (!id) return; UIC(id).min=(V2){(float)x,(float)y}; UIC(id).max=(V2){(float)(x+w),(float)(y+h)}; UIC(id).initialized=UIC(id).active=true; }
...
static u8 UIClicked(u32 id) {
    ...
    double* t=l?&UIC(id).lastLMB:&UIC(id).lastRMB; u8 dbl=(*t>0.0 && (World.pauseRelativeTime-*t)<=UI_DBLCLICK)?4u:0u; *t=dbl?0.0:World.pauseRelativeTime; return (u8)((l?1u:2u)|dbl);
}
...
/*---- Button action stubs. These are the interactions the Unity UI had wired up that the engine side has not implemented yet; every one is reached from UI_OnRegionClick so the call sites are live and the region already blocks the player. ----*/
void UI_ElevFloorClick(bool rh,int btn){(void)rh;(void)btn;}/*TODO ElevatorButton.cs ElevButtonClick()*/
void UI_ElevClose(bool rh){(void)rh;}/*TODO MFDManager.CloseElevatorPad()*/
void UI_KeycodeKey(bool rh,int k){(void)rh;(void)k;}/*TODO KeycodeButton.cs KeycodeButtonClick(), k 0-9 digits, UI_KEY_BACKSPACE, UI_KEY_CLEAR*/
void UI_KeycodeClose(bool rh){(void)rh;}/*TODO MFDManager.CloseKeycodePad()*/
void UI_AudioLogClick(bool rh){(void)rh;}/*TODO play/stop the tethered log*/
void UI_PuzzleGridCell(bool rh,int cell){(void)rh;(void)cell;}/*TODO PuzzleGrid.cs OnGridCellClick()*/
void UI_PuzzleGridSlide(bool rh,float f){(void)rh;(void)f;}/*TODO puzzle progress handle drag, f 0..1*/
void UI_PuzzleGridClose(bool rh){(void)rh;}/*TODO MFDManager.ClosePuzzleGrid()*/
void UI_WireNodeClick(bool rh,int node){(void)rh;(void)node;}/*TODO PuzzleWire.cs, node 0-6 LH column, 7-13 RH column*/
void UI_WireSlide(bool rh,float f){(void)rh;(void)f;}/*TODO wire level handle drag, f 0..1*/
void UI_WireClose(bool rh){(void)rh;}/*TODO MFDManager.ClosePuzzleWire()*/
void UI_SysAnalyzerClose(bool rh){(void)rh;}/*TODO SystemAnalyzerDisplay.Close()*/
void UI_MinigameStart(bool rh,int game){(void)rh;(void)game;}/*TODO MFDManager.MinigameStart_*(), 0 Ping..8 Chess*/
void UI_MinigameInput(bool rh,int x,int y){(void)rh;(void)x;(void)y;}/*TODO forward clicks into the running minigame view*/
void UI_MinigameBack(bool rh){(void)rh;}/*TODO MFDManager.OpenMinigames()*/
void UI_MinigameClose(bool rh){(void)rh;}/*TODO MFDManager.TabReset()*/
void UI_LogTableClick(int level){(void)level;}/*TODO open the per level folder, then MFD_READER_FOLDER*/
void UI_LogEntryClick(int entry){(void)entry;}/*TODO open entry, then MFD_READER_TEXT*/
void UI_LogMore(void){}/*TODO page the text reader forward*/
void UI_LogBack(void){}/*TODO back out of the text reader*/
void UI_EmailEntryClick(int entry){(void)entry;}/*TODO*/
void UI_DataEntryClick(int entry){(void)entry;}/*TODO*/
void UI_NoteToggleClick(int note){(void)note;}/*TODO tick/untick the objective note*/
void UI_HardwareRowClick(int idx){(void)idx;}/*TODO select hardware row, drive the side item panel*/
void UI_SoftwareRowClick(int idx){(void)idx;}/*TODO World.invP1.cyberItemIndex select*/
void UI_WeaponIconClick(bool rh){(void)rh;}/*TODO reload / alt ammo buttons on the weapon MFD*/
void UI_AutomapClick(bool rh,int action){(void)rh;(void)action;}/*TODO zoom in/out/full/side*/
...
void CenterMFDHeader() {
    ...
    if (World.inventoryMode && World.invP1.holdingObject) { UIR(UI_ID_CMFD_ADD_TO_INVENTORY,345,460,676,308); if (UIOver(UI_ID_CMFD_ADD_TO_INVENTORY)) { RenderUIImage(345,528,676,240,1075); RenderTextL(586,528,T_GREEN,FONT_NORMAL,1.0f,"ADD TO INVENTORY"); } }
    /*if (World.Sys_UI.showSensaroundCenter) { SensaroundCenter Plane } TODO*/
    if(World.Sys_UI.MFD_CenterTab==0) return;/*Tabs are off.*/
    if (World.Sys_UI.MFD_CenterTab!=1 && World.Sys_UI.MFD_CenterTab!=3) UIR(UI_ID_CMFD_PANEL,345,552,676,216);/*Opaque panels swallow the pointer wholesale; 1 and 3 hit test per row instead*/
    if(World.Sys_UI.MFD_CenterTab==1 && !Cheats.noHUD){/*MainTab: WeaponInventory,WeaponShotsInventory,GrenadeInventory,PatchInventory*/
        UIRText(UI_ID_CMFD_WEAPON_HEADER,372,560,T_RED,FONT_NORMAL,0.8f,200,"WEAPONS"); UIRText(UI_ID_CMFD_SHOTS_HEADER,574,560,T_RED,FONT_NORMAL,0.8f,120,"SHOTS");/*Column headers*/
        for(int slot=0;slot<7;++slot){
            int widx=World.invP1.weaponInventoryIndices[slot]; if(widx<0)continue; int y=582+slot*22;
            /*Slight shift of 6 feels better than just doing y and y + 22 as one would expect, then lopped 1 off one end to prevent double highlighting*/
            UIR(UI_ID_CMFD_WEAPON_ROW_0+slot,372,(i16)(y-5),340,21); bool hov=UIOver(UI_ID_CMFD_WEAPON_ROW_0+slot);
...
void SideMFDHeader(bool isRH) {
    ...
    for (u8 i=0;i<4;++i) UIRImg(MID(isRH,TAB_WEAPON)+i,isRH ? 1350 : -16,(i16)(520+56*i),32,40,selected==i+1 ? 1024 : 1022);/*Weapon/Item/Automap/Data side tab buttons*/
    if ((World.invP1.hardwareIsActive & HW_SNS) && World.invP1.hwVers[HW_SNS_IDX] > 1) {/*TODO Sensaround Plane*/ }
    if (tab == 0){return;}
    if (tab != 2) UIR(MID(isRH,PANEL),isRH ? 1022 : 24,520,320,248);/*Test BG fits 320x240 to match the 1:1 scale Doom's 320x200 would map to after 4:3 scaling*/
    if (tab == 1) {/*WeaponTab: WepNameText, WepIcon, ClipBox, EnergyHeatTicks, ReloadButtons, EnergySlider*/
        i16 slot=World.invP1.weaponCurrent; if (slot<0 || slot>=7) return; i32 widx=World.invP1.weaponInventoryIndices[slot]; if (widx < 0) return;
        UIRText(MID(isRH,WEAPON_NAME),isRH ? 1342 : 24,520,T_RED,FONT_NORMAL,0.8f,270,Sys_Text.stringTable[ItemStringIdx((i32)widx)]);/*Weapon Name*/
        if (wep16 >=0 && wep16 < 16) UIRImg(MID(isRH,WEAPON_ICON),isRH ? 1207 : 24,548,270,100,wepIconTexIndices[wep16]);/*WepIcon*/
...
void RenderConsumables(void) {
    for (int kind=0;kind<2;++kind) {
        i16 x=kind?831:734; UIRText(kind?UI_ID_CMFD_PATCH_HEADER:UI_ID_CMFD_GREN_HEADER,x,560,T_RED,FONT_NORMAL,0.8f,120,Sys_Text.stringTable[kind?873:872]);
        for (int row=0;row<7;++row) {
            ...
            UIRImg((kind?UI_ID_CMFD_PATCH_USE_0:UI_ID_CMFD_GREN_USE_0)+row,kind?920:795,(i16)(y+1),20,20,1086); RenderUIImage(kind?925:800,y+6,11,11,1079);
            UIR((kind?UI_ID_CMFD_PATCH_ROW_0:UI_ID_CMFD_GREN_ROW_0)+row,kind?831:706,y,kind?109:109,21);/*Row spans label+count+use, USE ids sort first so they win the overlap*/
        }
    }
}
...
void RenderGeneralInventory(void) {
    UIRText(UI_ID_CMFD_GENERAL_HEADER,454,557,T_RED,FONT_NORMAL,0.6f,260,Sys_Text.stringTable[875]);
...
static const i16 mgX[9]={32,32,32,32,156,156,156,156,32},mgY[9]={540,575,610,646,540,575,610,646,681}; static const char* mgName[9]={"Ping","15","Wing 0","Botbounce","Eel Zapper","Road","TriopToe","Corp Conq","Chess"};
...
void CenterMFD() {
    ...
    CenterMFDHeader();
    if (World.Sys_UI.MFD_CenterTab==1) RenderConsumables();/*Main*/
    if (World.Sys_UI.MFD_CenterTab==2) {/*Hardware*/
        UIRText(UI_ID_CMFD_HARDWARE_HEADER,454,557,T_RED,FONT_NORMAL,0.8f,260,874<1100?Sys_Text.stringTable[874]:"HARDWARE");
        for (int i=0;i<HW_COUNT && i<12;++i) { int ref=World.invP1.hardwareInvReferenceIndex[i]; if (ref<0 || World.invP1.hwVers[i] <= 0) continue;
            i16 x=(i16)(458+(i/6)*223), y=(i16)(575+(i%6)*23);/*2 columns of 6*/
            ...
            UIRText(UI_ID_CMFD_HARDWARE_ROW_0+i,x,y,World.invP1.hardwareInvCurrent==i?T_YELLOW:(World.invP1.hasHardware&(1u<<i)?T_GREEN_MENU:T_GREEN_MENU_SHADOW),FONT_NORMAL,sc,210,label);
            RenderTextL((i16)(x+195),y,World.invP1.hardwareInvCurrent==i?T_YELLOW:T_GREEN_MENU,FONT_NORMAL,0.5f,"v%d",(int)World.invP1.hwVers[i]); }
    }
    if (World.Sys_UI.MFD_CenterTab==3) RenderGeneralInventory();
    if (World.Sys_UI.MFD_CenterTab==4) {/*Software*/
        UIRText(UI_ID_CMFD_SOFTWARE_HEADER,454,557,T_RED,FONT_NORMAL,0.8f,260,876<1100?Sys_Text.stringTable[876]:"SOFTWARE");
        for (int i=0;i<7;++i) { bool owned; int count=0;
            ...
            if (!owned) continue;
            i16 y=(i16)(588+i*32); const char* label=swLabels[i]; bool selected=i==World.invP1.cyberItemIndex; float w=MeasureLineAdvance(label,FONT_NORMAL),sc=w>0?vmin(0.6f,210.0f/w):0.6f;
            UIRText(UI_ID_CMFD_SOFTWARE_ROW_0+i,454,y,selected?T_YELLOW:T_GREEN_MENU,FONT_NORMAL,sc,210,label);
            if (i<=2) RenderTextL(680,y,selected?T_YELLOW:T_GREEN_MENU,FONT_NORMAL,0.5f,"v%d",World.invP1.softVersions[i]+1); else if (i<=5) RenderTextL(680,y,selected?T_YELLOW:T_GREEN_MENU,FONT_NORMAL,0.5f,"x%d",count); else RenderTextL(680,y,selected?T_YELLOW:T_GREEN_MENU,FONT_NORMAL,0.5f,"%d",World.invP1.hasMinigame?1:0); }
    }
    if (World.Sys_UI.MFD_CenterTab!=5) return;/*EReader*/
    UIRText(UI_ID_CMFD_MEDIA_HEADER,454,557,T_RED,FONT_NORMAL,0.6f,260,877<1100?Sys_Text.stringTable[877]:"LOGS");
    if (World.Sys_UI.MFD_MediaTab==MM_LOG_TABLE) {
        if (World.Sys_UI.MFD_ReaderView==MFD_READER_CONTENTS) { RenderUIImage(454,573,453,191,0);/*LogTableofContents*/
            for (u8 i=0;i<10;++i) { i16 x=(i16)(i<7?454:681),y=generalRowY[i%7]; UIRImg(UI_ID_CMFD_LOG_TABLE_0+i,x,y,226,24,0);/*QUAD:builtin-white*/ RenderTextL(x,y,T_GREEN,FONT_NORMAL,0.6,"Level %s Logs",logLevelName[i]); RenderTextL((i16)(x+77),y,T_GREEN,FONT_NORMAL,0.6,"3");/*TODO real per level counts*/ }
        } else if (World.Sys_UI.MFD_ReaderView==MFD_READER_FOLDER) { RenderUIImage(458,570,445,188,0);/*LogsLevelFolder*/
            for (u8 i=0;i<15;++i) { i16 x=mediaColX[i/8],y=mediaRowY[i%8]; UIRImg(UI_ID_CMFD_LOG_ENTRY_0+i,x,y,222,21,0); RenderTextL(x,y,T_GREEN,FONT_NORMAL,0.6,"Log"); }
        } else if (World.Sys_UI.MFD_ReaderView==MFD_READER_TEXT) {
            UIR(UI_ID_CMFD_LOG_TEXT,449,576,456,140); RenderTextL(449,576,T_GREEN,FONT_NORMAL,0.6,"\"abc def ghi jkl mno pqrs tuv wxyz ABC DEF GHI JKL MNO PQRS TUV WXYZ !\"\\xA7\n$%%& /() =?* '<> #|; \\xB2\\xB3~ @`\\xB4 \\xA9\\xAB\\xBB \\xA4\\xBC\\x...");
            UIRImg(UI_ID_CMFD_LOG_MORE,454,576,456,174,0); RenderTextL(654,647,T_YELLOW,FONT_NORMAL,0.6,"%s",26<1100?Sys_Text.stringTable[26]:"[MORE]");
            UIRImg(UI_ID_CMFD_LOG_BACK,453,718,69,31,0); RenderTextL(453,718,T_YELLOW,FONT_NORMAL,0.6,"%s",879<1100?Sys_Text.stringTable[879]:"[BACK]");
        }
    }
    if (World.Sys_UI.MFD_MediaTab==MM_EMAIL_TABLE) { RenderUIImage(458,570,445,188,0);/*EmailTab*/
        for (u8 i=0;i<15;++i) { i16 x=mediaColX[i/8],y=mediaRowY[i%8]; UIRImg(UI_ID_CMFD_EMAIL_ENTRY_0+i,x,y,223,21,0); RenderTextL(x,y,T_GREEN,FONT_NORMAL,0.6,"Email"); }
    }
    if (World.Sys_UI.MFD_MediaTab==MM_DATA_TABLE) { RenderUIImage(458,570,445,188,0);/*DataTab*/
        for (u8 i=0;i<13;++i) { i16 x=mediaColX[i/8],y=mediaRowY[i%8]; UIRImg(UI_ID_CMFD_DATA_ENTRY_0+i,x,y,223,21,0); RenderTextL(x,y,T_GREEN,FONT_NORMAL,0.6,"Data"); }
    }
    if (World.Sys_UI.MFD_MediaTab==MM_NOTES) {
        ...
        for (u8 i=0;i<18;++i) { i16 x=(i16)(453+(i/6)*167),y=(i16)(570+(i%6)*29); UIR(UI_ID_CMFD_NOTE_TOGGLE_0+i,x,y,166,39); RenderUIImage(x,(i16)(y+2),19,18,910);/*Background*/
            if (i<6) RenderTextL((i16)(x+21),(i16)(y+3),T_GREEN,FONT_NORMAL,0.6,"%s%d%s%s%u.",Sys_Text.stringTable[556],i+1,Sys_Text.stringTable[557],Sys_Text.stringTable[558],secCode[i]); else RenderTextL((i16)(x+21),(i16)(y+3),T_GREEN,FONT_NORMAL,0.6,"%s",noteText[i-6]); }
    }
}
...
static double RenderUI() {
    ...
    if (World.menuActive || World.paused) {
        ...
        if (!Cheats.noHUD) {
            ...
            if (World.curLev==LEVEL_CYBERSPACE) { UIR(UI_ID_CMFD_CYBER_TIMER,28,530,80,14); RenderTextL(28,530,T_WHITE,FONT_NORMAL,0.6,"T -"); RenderTextL(68,530,T_WHITE,FONT_NORMAL,0.6,"99:99"); }
            ...
            if((World.invP1.hardwareIsActive & HW_BIO)!=0){/*BioMonitor*/
                ...
                RenderTextL(4,83,T_YELLOW,FONT_NORMAL,0.6,"%s",895<1100?Sys_Text.stringTable[895]:"Biomonitor:"); RenderTextL(4,99,T_GREEN,FONT_NORMAL,0.6,"%s",896<1100?Sys_Text.stringTable[896]:"Heart Rate:");
                RenderTextL(70,99,T_GREEN,FONT_NORMAL,0.6,"100"); RenderTextL(122,99,T_GREEN,FONT_NORMAL,0.6,"BPM");
                RenderTextL(4,131,T_GREEN,FONT_NORMAL,0.6,"%s",897<1100?Sys_Text.stringTable[897]:"Patches Active:"); RenderTextL(119,131,T_GREEN,FONT_NORMAL,0.6,"MEDI STAMINUP SIGHT GENIUS BERSERK REFLEX");
                RenderTextL(4,115,T_GREEN,FONT_NORMAL,0.6,"%s",898<1100?Sys_Text.stringTable[898]:"Fatigue:"); RenderTextL(66,115,T_GREEN,FONT_NORMAL,0.6,"Moderate");
            }
            RenderTextL(1270,78,T_WHITE,FONT_NORMAL,0.6,"0"); RenderTextL(1308,78,T_WHITE,FONT_NORMAL,0.6,"0"); RenderSearchFX();
        }
